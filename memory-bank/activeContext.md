...
>node-properties-graph-selection::IMPLEMENTED✓{2026-07-15,commits:a2eeba7b+bc8c8377+c62ef868;user-verified-selection+pin-edges}
  ...
  interactive-resize{042d7c13,user:resize-works}::drag-node-edges/corners-resizes{sizing-cursors-on-hover;right/bottom/BR=resize-in-place,left/top+other-corners=resize+reposition;interior-drag-still-moves;drives-ui_width/height→persists}
    cursor-fix{040e6f18,AWAITING-user-re-verify}::SDL-set_cursor-was-empty-stub{NO-ImGui-cursor-shape-ever-reached-OS-on-default-SDL-backend}→implemented{SDL_CreateSystemCursor-per-shape+set_cursor;Mouse_cursor-enum-aligned-to-ImGui{Wait/Progress-inserted,NotAllowed=10,Crosshair=11}across-sdl/glfw/null-headers}
    vendored-editor::SizeAction-accepts-plain-nodes-when-EditorContext::EnableNodeResize(true){GetRegion/GetRegionBounds-give-plain-nodes-group-style-edge-regions{6cu-strip-inside-NodePadding};interior=Header;Size-before-Drag-in-accept-chain;plain-nodes-skip-group-min-size-term{group-bounds-empty-would-pin-start-size}}
    app::GetNodeResize(nodeId,pos,size)-polled-after-canvas-End→apply_node_resize-in-Graph_editor_window_base{find_graph_editor_node-virtual}+inline-in-Graph_window;EnableNodeResize-in-all-3-windows
