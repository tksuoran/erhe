...
verification of doc/editor/gltf_scene_roundtrip.md:
...
6. Round-trips USD content on its own leg (doc/erhe/usd_compatibility_design.md
...
def norm_quat(values, digits=4):
    ...
    A prim outside Xformable carries no transform (doc/erhe/usd_compatibility_design.md
...
def norm_node(node, parent_name=None):
    return {
        "name":        node.get("name"),
        # The prim's erhe class (doc/erhe/usd_compatibility_design.md C5): a prim
...
# (doc/erhe/usd_compatibility_design.md U4). They are prims of the scene tree, so
...
def normalize_import_roots(raw_nodes):
    ...
    are written in their place; doc/editor/scene_serialization.md save pipeline
...
def snapshot_scene(scene_name, material_names, detail_nodes):
    ...
    # A local value is an authored value (doc/erhe/property_system.md D32): a
...
def section_build_scene():
    ...
    def block_resource_placement():
        ...
        # (doc/erhe/usd_compatibility_design.md C5); ERHE_scene library_folders
        ...
        # writes an unused library material (doc/editor/asset_manager.md).
...
def section_reload_and_diff():
    ...
    # the animated value layer (doc/erhe/property_system.md D5) over the transform
...
# --------------------------------------------------------------------------
# Section 6: USD round trip (doc/erhe/usd_compatibility_design.md E3)
# --------------------------------------------------------------------------
...
def usd_snapshot(scene_name):
    ...
    # (doc/erhe/usd_node_graphs.md): every graph asset with its nodes,
    ...
    # The variant sets the scene carries (doc/erhe/usd_compatibility_design.md X4):
    ...
    # A local value is an authored value (doc/erhe/property_system.md D32): the
    ...
    # arc (doc/erhe/usd_compatibility_design.md X3) are diffed like any other value.
...
def usd_library_folder_leg(S):
    ...
    (doc/editor/content_library_folders.md) and a USD file carries every `Scope` it is
    given (doc/erhe/usd_compatibility_design.md E4d): a folder holding a material and
    ...
    # ones (doc/erhe/usd.md, sanitize_usd_identifier).
...
def usd_physics_leg(S):
    """The physics a file carries (doc/erhe/usd_compatibility.md, "Physics"):
    ...
    # (doc/erhe/usd.md, sanitize_usd_identifier).
    material = mutate("create_physics_material", {
...
def usd_references_leg(S):
    """A reference with sparse overrides (doc/erhe/usd_compatibility_design.md X2):
...
def usd_resource_placement_leg(S):
    """A resource is a prim of the scene tree and may sit under any prim
    (doc/erhe/usd_compatibility_design.md C5, U4); in USD the tree is the file, so a
...
def usd_geometry_graph_leg(S):
    """A geometry graph in a USD file (doc/erhe/usd_node_graphs.md section
...
def section_usd_round_trip(usdchecker_arg):
    ...
    # (doc/erhe/usd_compatibility_design.md X3): the classes must come back as Style
    ...
    # (doc/erhe/usd_compatibility_design.md X4): the table, the selection and the
    ...
    # (doc/erhe/usd_compatibility_design.md E4a): the brushes must come back with
    ...
    # (doc/erhe/usd_compatibility_design.md K1). The skeleton comes back as the
    ...
    # place of an image (doc/erhe/usd_node_graphs.md): the graph must come
    ...
    # (doc/erhe/usd_compatibility_design.md E2). An anisotropic roughness and a
...
# --------------------------------------------------------------------------
# glTF material variants (doc/erhe/usd_compatibility_design.md X4)
# --------------------------------------------------------------------------
