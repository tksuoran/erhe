...
1. **Non-multiview regression** on each backend (OpenGL Windows /
   ...
   content wide lines and debug lines pixel-match baseline. Confirms
   shader and buffer layout changes are no-ops at `view_count = 1`.
2. **Quest 3 + Vulkan**: launch in VR, confirm log line "OpenXR
   multiview swapchain created: WxH, 2 views, ..." appears and
   `Xr_session::is_multiview_enabled()` returns true.
3. **Stereo correctness on device**: each eye sees the correct image
   with IPD parallax; no flickering, no swapped eyes.
4. **Wide / debug line per-eye correctness**: spawn known debug lines
   (axes, frustum); compare each eye against the per-eye baseline
   render. Widths and positions must match.
5. **Capability fallback**: force the Vulkan multiview cap to false;
   confirm `Headset_view` takes the per-view path with no shader
   recompile errors and no rendering regressions.
