...
> **Status (2026-05-19): wiring renamed; behaviour intact.** Multiview still
> runs end-to-end on Quest 3 via Vulkan, but the C++ surface changed:
>
> - `Shader_stages_create_info::enable_multiview(N)` is removed. Set
>   `view_count` directly on the create_info (1 = single-view, >= 2 =
>   multiview). `final_source()` always emits `ERHE_VIEW_COUNT` and emits
>   the `GL_EXT_multiview` `#extension` plus `ERHE_MULTIVIEW` define when
>   `view_count > 1`.
...
>   vertex_input, vertex_format, and render-pass format/usage). The
>   `multiview_shader_stages` sibling-pointer and `uses_standard_variants`
>   flag are gone. `Lazy_render_pipeline` now takes a `Base_` and resolves
>   shader_stages / vertex_input / vertex_format at the call site via
>   `get_pipeline_for(render_pass_desc, shader_stages, vertex_input,
>   vertex_format)`.
> - `Render_command_encoder::set_render_pipeline_state(state,
>   override_shader_stages)` is gone. The variant selection moves to the
>   caller, which picks the right `Shader_stages*` (single-view or
>   multiview) before calling `get_pipeline_for(...)`. Forward_renderer's
>   `override_shader_stages` / `multiview_views` / `standard_shader_variants`
>   parameters are also gone; multiview cameras now ride on the single
>   `views` span.
> - `Programs::get_multiview()` and the parallel multiview-shader map are
>   retired. Each consumer that needs both single-view and multiview shaders
>   keeps its own pair locally (e.g. `Text_renderer::m_multiview_shader_stages`).
>
> Read the body of this document for the high-level data flow and capability
> gate, which are still accurate; ignore references to the removed API
> names above when reading source.
...
- Vulkan backend only. The OpenGL backend does NOT support OpenXR
  ...
  and OpenXR-on-GL stays out of scope. (GL_EXT_multiview / OVR_multiview2
  are not wired into the renderer's view-count plumbing.)
