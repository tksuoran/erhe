...
Vulkan 1.3. It is feature-complete: the `editor`, `example`, `hello_swap` and
`hextiles` executables run on it, and it is the backend used for the
...
all backends, see `doc/erhe/graphics.md`; for the Metal backend see
`doc/erhe/metal_backend.md`.
...
- [Vulkan backend known issues](../plans/vulkan_backend.md) - rounding the
  ID-buffer edge-line end caps in the fragment shader.
