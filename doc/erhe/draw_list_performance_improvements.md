...
is per `Scene_root`; `doc/erhe/draw_list_renderer.md` owns the renderer those lists
...
| `material_index` | the material's slot in the draw-list `Material_set` (`doc/erhe/draw_list_material_set.md`) | Baked at registration and kept by the material set: a slot is stable for as long as anything references the material in that set, so the record needs no per-pass repair |
...
- These records are what R12b of `doc/erhe/draw_list_renderer.md` means by a hook
