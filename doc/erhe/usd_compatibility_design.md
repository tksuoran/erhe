...
`doc/erhe/usd_compatibility.md` (referred to below as "the mapping"); this
...
- L1 LightUSD as an optional CPM dependency: `ERHE_USD_LIBRARY`
  ...
  and the Android build pass `lightusd` (`doc/erhe/usd.md`,
...
- The `tksuoran/LightUSD` fork carries the behavior erhe needs beyond the
  ...
  connection it does not model (`doc/erhe/usd.md` "Node graphs"),
...
- E1 Save as USDA: a USD-backed scene (`Scene_root::get_source_format()`)
  ...
  block as `customLayerData` (`doc/erhe/usd.md` "Export",
  `doc/editor/scene_serialization.md` "USD-backed scenes").
...
- Q1 Quest build and launch: the `quest` flavor builds with the option on
  ...
  and build-time cost are tabulated in `doc/erhe/usd.md`
  "Configurations".
- U1 Prim class hierarchy: `erhe::Typed` and `erhe::Scope`
  (`doc/erhe/item.md` "Prim classes"), `erhe::scene::Imageable`,
  `Xformable` (today's `Node`, alias kept), `Xform`, `Boundable` and
  `Gprim` (`doc/erhe/scene.md`); a transform composes with the
  ...
  glTF carries `Scope` / `Typed` on `ERHE_node` (`doc/erhe/usd.md`
...
- U2 Mesh is a Gprim: `erhe::scene::Mesh` is `erhe::Item<Item_base,
  ...
  (`doc/erhe/usd.md`); `Xformable`'s secondary owner type is
  `Item_base` (`doc/erhe/property_system.md` D30).
- U3 Camera and Light are Xformables: `Camera` and `Light` are
  ...
  for `Node_physics`, `Node_joint`, `Prefab_instance`
...
- U4 Resources are prims: every content-library kind is `erhe::Item<
  ...
  `Item_host::register_prim` (`doc/erhe/item.md`,
  `doc/editor/content_library.md`); a folder is a `Scope`
  (`doc/editor/content_library_folders.md`); resources enter and move through
  ...
  (`doc/editor/asset_manager.md`); glTF carries a resource's tree position in
  `ERHE_scene` `library_folders` (`doc/gltf_extensions/ERHE_scene.md`)
  and USD writes and reads a `Material` prim where it sits
  (`doc/erhe/usd.md`); the other resource kinds and a folder are E4.
...
- M6 The value types USD needs: `Property_type::double_floating`,
  ...
  (`doc/erhe/property_system.md` D2, D16), written and read as USD `double`,
  `matrix4d`, `asset`, `float[]` and `int[]` attributes
  (`doc/erhe/usd_compatibility.md` Property-system rows);
...
- M7 Style chains: a style has a style of its own; the style layer is
  ...
  refuses a cycle (`doc/erhe/property_system.md` D25, `doc/editor/style_library.md`);
...
- M8 xformOp stacks: an `Xformable` carries the xformOp stack it was
  ...
  (`doc/erhe/scene.md`); the USD reader builds the stack from the
  raw prim and the writer emits it as authored, falling back to one
  `xformOp:transform` for a prim without a stack
  (`doc/erhe/usd.md`, `doc/erhe/usd_compatibility.md`). `erhe_usd_tests`
...
- Transform time samples: a time-sampled `xformOp:*` attribute carries its
  ...
  point (`doc/erhe/usd.md`, "Time samples").
- X1 References as prefab instances: LightUSD composes nothing at load
  ...
  point (`doc/erhe/usd.md` "Export"). A typeless or `Scope` prim
...
- X2 Editable instances with sparse overrides: a USD reference is a
  ...
  (`doc/erhe/property_system.md` D33, R3 is coerced, local, style,
  ...
  states once what an override is (`doc/erhe/scene.md`); a USD
...
- X3 Class inheritance: a `class` prim is a Style item. The reader takes
  ...
  one warning each (`doc/editor/parsers.md`). The writer inverts it:
  ...
  (`doc/erhe/usd.md`; the mapping's style rows). An `over` prim
...
- X4 Variants: material bindings and property opinions. The reader
  ...
  (`doc/erhe/usd.md` "Variant sets"). The editor keeps one
  ...
  blocks and the selection back (`doc/editor/scene.md`,
  `doc/editor/scene_serialization.md`). On the glTF side `KHR_materials_variants`
  ...
  scene block (`doc/erhe/gltf.md`); it carries bindings only, so a
...
- K1 Skinning: a `Mesh` with the `SkelBindingAPI` is skinned the way a
  glTF mesh with a skin is, and saves back with it (`doc/erhe/usd.md`
...
- C6 Composition the real assets use, three forms erhe's own
  ...
  `doc/erhe/scene.md`). Teapot.usd imports its 1 mesh and 2
...
- C7 Variant selection through a composition arc. A prim that references
  ...
  the Fancy geometry (`doc/erhe/usd.md` "Variant sets", "Composition
  arcs"; `doc/editor/parsers.md`).
- C8 A `UsdPreviewSurface` input fed by a `UsdPrimvarReader`. Tydra
  ...
  `PyramidMaterial` convert (3 of 3 there; `doc/erhe/usd.md`
  "UsdPreviewSurface fallbacks and channel outputs";
  `doc/erhe/scene_renderer.md`). A material converted after Tydra's
...
- C10 `GeomModelAPI` draw modes. A model prim's `UsdGeomModelAPI` is a value
  ...
  (`doc/editor/scene.md` "Draw modes"; `doc/erhe/usd.md`
  "Draw modes"; `doc/erhe/item.md`).
...
- E4 Editor state in a USD file: every resource kind of the content
  ...
  (`doc/editor/scene_serialization.md`, "USD-backed scenes", owns the list of what
  ...
  - Brushes: a brush is a `Brush`-typed prim where it sits, its geometry
    ...
    (`doc/erhe/usd.md` "Brush prims"). The round-trip script's
  ...
  - Texture node graphs: a `Graph_texture` is the `UsdShade` network it
    ...
    output in place of a `UsdUVTexture`. `doc/erhe/usd_node_graphs.md`
    owns the design and the record between `erhe::usd` and the editor
    (`doc/erhe/usd.md` "Node graphs"; MCP `get_scene_node_graphs`);
  ...
  - Geometry node graphs: a `Graph_mesh` reuses the texture-graph prim
    ...
    `doc/erhe/usd_node_graphs.md` section 4 states the rule, the
  ...
  - Folders: every `Scope` of the tree is written where it sits, whatever
    ...
    (`Content_library::adopt_kind_scopes`, `doc/editor/parsers.md`).
...
- S1 USD Assets Working Group survey: every entry asset of the ASWF USD
  ...
  renderers is stated where it holds: `doc/erhe/usd.md` for the
  ...
  stacks and the specifier of an undefined prim; `doc/editor/rendering.md`
...
- E2 Material fidelity: the erhe material fields `UsdPreviewSurface` has no
  ...
  `erhe:Material:<name>` custom-attribute path (`doc/erhe/usd.md`
...
- P1 Physics in USD: the physics of a scene travels as the `UsdPhysics`
  ...
  (`doc/erhe/scene.md`, "Physics description") is the format-neutral
  ...
  writers consume (`doc/editor/parsers.md`). A body is its prim's
  ...
  custom attribute (`doc/erhe/usd.md`, "Physics" under Import and
...
- An animation plays through a value layer of its own and an edited clip
  ...
  layer of `doc/erhe/property_system.md` D5, so the transform a prim authored
  ...
  (`doc/erhe/usd.md`, "Time samples"). `erhe_usd_tests` edits a key
...
- Time samples beyond the transform. An `erhe::scene::Animation_channel`
  names the property it drives (`doc/erhe/scene.md`, "Animation
  ...
  the mapping; `doc/erhe/usd.md`, "Time samples"). The samples are
...
- A load of a stage holding thousands of prims costs the content it
  ...
  `App_context::is_scene_load_in_flight()`, `doc/editor/scene.md`),
  ...
  and N-1 that refresh their own mesh alone (`doc/editor/async_asset_loading.md`, "Deferred load
  ...
  last breadcrumb the tick happened to pass (`doc/editor/parsers.md`).
...
- OpenUSD as a build dependency (`doc/erhe/usd.md` "Dependency"
