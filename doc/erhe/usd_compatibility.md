...
(`doc/erhe/usd_compatibility_design.md` C5) and still carries some object kinds as
...
| `Xform` with several applied-API-schema attachments | `Xform` with those schemas applied to it | what USD applies to a prim as an API schema is what erhe attaches to a node |
