...
| `Xform` with several applied-API-schema attachments (`Node_physics`, `Node_joint`, `Prefab_instance`, `Frame_controller`, `Grid`) | `Xform` with those schemas applied to it | what USD applies to a prim as an API schema is what erhe attaches to a node |
