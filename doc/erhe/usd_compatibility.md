...
| `Xform` with several applied-API-schema attachments (`Prefab_instance`) | `Xform` with those schemas applied to it | what USD applies to a prim as an API schema is what erhe attaches to a node |
...
| prefab instance (`Prefab_instance`, glTF 2.1 externalAssets) | `references` (or `payload`) composition arc | one attachment per arc, in the authored order - the prim's own arcs first, then the ones the selected variant of each of its variant sets authors; the arc's target file, prim path and form are what the attachment records, and a save writes them back where they were authored, a variant's inside its block. The carrier's own `variants` metadatum names the selection it carries into the target - one entry per variant set, keyed by the path below the target prim, an empty path being that prim - and the attachment records it, so a save authors it back on the carrier and on the `over` prim of a deeper path. An `xformOpOrder` entry the prim does not author is taken from the first internal arc target that does (an arc into another file is not followed), and an override path looks through the clone of every carrier it crosses (C6) |
