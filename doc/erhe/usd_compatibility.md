...
`doc/editor/scene_serialization.md`) and is growing USD into a second,
...
scope is `doc/erhe/usd_compatibility_design.md`, and the work that is left is
...
(`doc/erhe/usd.md`). Per-machine clone locations are recorded in
`memory-bank/local/`.
...
| content library (per-scene index of the scene's resources) | the prim tree itself | a resource is a prim where it sits; the library only indexes them (`doc/editor/content_library.md`). A folder is a `Scope`, and each kind has one `Scope` below the scene root (`/Materials`, `/Brushes`, ...) - where a resource goes when nothing else places it. glTF carries the position in `ERHE_scene` `library_folders` (`doc/gltf_extensions/ERHE_scene.md`), where a path names any prim of the tree; in USD the tree is the file, so the prim's place in the layer is the position, and the `Material` prim row of "Object model" says which kinds a USD file carries so far |

## Object model

USD has one typed prim per object and erhe has the same shape
(`doc/erhe/usd_compatibility_design.md` C5): what USD applies to a prim as an
API schema is a group of values of the erhe prim itself. The mapping an
exporter applies and an importer inverts:

| erhe | USD | notes |
|---|---|---|
| `Xform` (transform + children) | `Xform` (`UsdGeomXformable`) | an `Xformable` holds the xformOp stack it was authored with (`erhe::scene::Xform_op_stack`) next to the single T\*R\*S the stack composes to, so an imported stack - op types, suffixes, `!invert!` flags, authored value precisions and `!resetXformStack!` - is written back as authored; an edit lands in the op the stack designates (`doc/erhe/scene.md`, authored xformOp stacks). A prim erhe created carries no stack and writes one `xformOp:transform`. glTF carries the composed T\*R\*S alone |
| `Scope` (children only, no transform) | `Scope` | a transform composes through it to the nearest transformable ancestor. A content-library folder is one, and so is each kind's scope (`/Materials`, `/Brushes`, ...); every `Scope` a USD file authors round-trips as itself, the scope holding a stage's `Material` prims included. Every `Scope` of the tree is written where it sits, whatever it holds, so an empty folder and an empty kind scope are prims of the saved layer and the folder tree survives the save (`doc/erhe/usd_compatibility_design.md` E4d). A kind scope is recognized on reload by its name, in the identifier spelling the stage carries |
...
| the node's `Node_physics.*` values / an `editor::Joint` prim | `UsdPhysics` API schemas / joint prims, see "Physics" | |
| `Skin` | `UsdSkel` (`SkelRoot`, `Skeleton`, `SkelBindingAPI`) | the skin's pivot is the `Skeleton` prim and its joints are `Xform` prims below it, so a joint animates the way any prim does; `inverse_bind_j` is `inverse(bind_j) * geomBindTransform` and a save splits it back with an identity `geomBindTransform` for the skeleton's first skin (`doc/erhe/usd.md`, "Skinning") |
| `editor::Brush` | `Brush` (custom `typeName`, no USD schema) | a brush is a prim where it sits: `erhe:Brush:density` and `erhe:Brush:normal_style` custom attributes, `material:binding` for the material a placed instance gets, and the geometry as a child `def Mesh "geometry"` with `subdivisionScheme = none` (`doc/erhe/usd.md`, "Brush prims"). `purpose` is derived from the brush flag (M3), so it is not authored |
...
| `Layout` (a value group of the node, `doc/erhe/property_system.md` section 4.13), `Brush_placement` (a value group of the node, section 4.11, never written), `Grid`, `Rendertarget_mesh` | custom (codeless) schemas or namespaced custom attributes (`erhe:...`) | editor domain, no USD counterpart; the attribute form is the qualified-name row of "Property system" |
| prefab instance (`erhe::Composition_arc`, glTF 2.1 externalAssets) | `references` (or `payload`) composition arc | one record per arc, in the authored order - the prim's own arcs first, then the ones the selected variant of each of its variant sets authors; the arc's target file, prim path and form are what the record holds, and a save writes them back where they were authored, a variant's inside its block. The carrier's own `variants` metadatum names the selection it carries into the target - one entry per variant set, keyed by the path below the target prim, an empty path being that prim - and the record holds it, so a save authors it back on the carrier and on the `over` prim of a deeper path. An `xformOpOrder` entry the prim does not author is taken from the first internal arc target that does (an arc into another file is not followed), and an override path looks through the clone of every carrier it crosses (C6) |
...
erhe's property system (`doc/erhe/property_system.md`) is the part of erhe
...
| node-held values of another item class (D30, `Light.color` on an empty node) | same as folder-held values | |
...
| `texcoord_0..2` | `primvars:st`, `primvars:st1`, `primvars:st2` (`texCoord2f[]`) | `texcoord_2` is the lightmap UV set. The two conventions put the image origin in opposite corners - USD's `st` at the bottom-left, erhe's (glTF's) at the top-left - so every texcoord is read and written through `v' = 1 - v`; the formula and what it does to a `UsdTransform2d` are stated once in `doc/erhe/usd.md`, "Texture coordinates" |
| `color_0..1` | `primvars:displayColor` (+ `displayOpacity`), `primvars:color1` | a `displayColor` that varies is vertex color data and lives here; a constant one is the whole surface's color and is the `Gprim.display_color` row below |
| `Gprim.display_color` (a prim property, not an attribute of the geometry) | `primvars:displayColor` at `interpolation = "constant"` | the one color of the whole surface: read as a local value when a single-element `displayColor` is authored, and written back as the same constant primvar in place of the per-corner array. The value also reaches the vertex data, because that is where every renderer reads a mesh's own color from, so a write rebuilds the mesh's primitives with it (`doc/erhe/scene.md`). Native in both prim forms - a typeless `over` carries a primvar as readily as a `Mesh` prim does - so it never also rides as an `erhe:` custom attribute |
...
| `base_color_source`, `opacity_source` | a `UsdPrimvarReader` on `inputs:diffuseColor` / `inputs:opacity` | a reader of `displayColor` (`displayOpacity` for `opacity`) makes the input the mesh's vertex colors alone - the erhe factor and the slot's texture are not read - and is written back as the reader prim and the connection in place of a value, so nothing of that input is a local value. Any other primvar is one warning and the input keeps its own value. Tydra fails a material over a connection it cannot resolve to a `UsdUVTexture`, so `load_stage` takes these connections out of the layer the stage is built from and hands them to the importer (`doc/erhe/usd.md`). glTF has no form for either, so a glTF file always reads `value` |
...
| `<slot>_texture_uv_*` | `UsdTransform2d` | USD composes `in * scale`, then the rotation, then the translation, which is the order the erhe slot transform applies. The erhe transform acts on the flipped texcoord, so the two are converted through `v' = 1 - v` in both directions (`doc/erhe/usd.md`, "Texture coordinates"). A save authors a `UsdTransform2d` prim between the material's primvar reader and the slot's `UsdUVTexture` for exactly the slots whose transform is not the identity |
...
An erhe texture graph (`editor::Graph_texture`, `doc/editor/texture_graph.md`)
...
| a node parameter | `inputs:<name>` carrying a value | typed from the parameter's own kind: `float`, `int`, `bool`, `float2` (a size), `color3f` / `color4f` (a color), `string` for a name. A value with no USD form - a gradient, a curve - travels as its JSON text in a `string`, as it stands, one rule for both |
...
| the proxy the editor draws | none - `UsdImagingGLDrawModeAdapter` | `bounds` is the 12 edges of the extent box and `origin` three axis lines from the prim's origin, in the prim's own space, depth-tested like the geometry they stand for. `cards` is generated quad geometry: one `Mesh` child prim of the model prim with one primitive per drawn face and one unlit, single-sided material each (the two faces of an axis face away from each other; a `cross` pair shares the mid plane and back-face culling picks the visible one) - the face's `cardTexture` on white, or `draw_mode_color` when the face names none. A textured face is cut out along the image's alpha channel, which is where the card image carries the silhouette of the subtree it stands for: the material is `alpha_test` with the cutoff 0.1, reading the alpha of the base color slot, which is what `UsdImagingDrawModeAdapter` asks for when it connects the card texture's alpha to `opacity` and sets `opacityThreshold`. An untextured face is opaque over its whole quad. `cross` puts the two faces of an axis on the extent's mid plane, `box` on the extent's own planes, in the adapter's own corner order and with its unflipped UV quad taken through erhe's `v' = 1 - v`; `fromTexture` places each face by the `worldtoscreen` matrix in its image's PNG text chunk and falls back to `box` for an image carrying none, with one warning per prim. `card_visibility = simple` leaves the Y pair out. The proxy carries `Item_flags::draw_mode_proxy` (so the prim's own pruning does not reach it, and a viewport pick of it selects the model prim) and `Item_flags::session_only` (so no exporter writes it); the item tree does not list it, and it is rebuilt whenever a draw-mode value of the prim or the extent changes. An inactive prim owns no proxy - the derived `Item_flags::active` bit going out takes it away and the bit coming back builds it, so the clones an instance puts below a pruning prim carry no cards nothing draws |
...
|---|---|---|
| `Node_physics.motion_mode` on a prim | `PhysicsRigidBodyAPI` applied to the prim; the body's colliders are the prims at or below it carrying `PhysicsCollisionAPI` | the body prim of a collider is its nearest ancestor-or-self with `PhysicsRigidBodyAPI`; a collider with no such ancestor is a static body on its own prim. Colliders below a body fold into one compound shape, as `KHR_physics_rigid_bodies` colliders do |
| `motion_mode` static / kinematic / dynamic | `PhysicsCollisionAPI` without `PhysicsRigidBodyAPI` / `PhysicsRigidBodyAPI` with `physics:kinematicEnabled = true` / `PhysicsRigidBodyAPI` | `physics:rigidBodyEnabled = false` reads as static. The schemas spell three of the five modes, so a body in `e_kinematic_non_physical` - the one mode they cannot tell from `e_kinematic_physical` - also carries `erhe:Node_physics:motion_mode`, and the import takes that value while it names the same one of the three states |
| `mass`, `center_of_mass_offset`, inertia | `PhysicsMassAPI` `physics:mass`, `physics:centerOfMass`, `physics:diagonalInertia`, `physics:principalAxes` | erhe's density-derived default mass = `physics:mass = 0` (the schema default); an unauthored `centerOfMass` (`-inf`) is the body's own. The writer authors these for a body with a motion record, so a static body's local `mass` or `center_of_mass_offset` travels as `erhe:Node_physics:<name>` instead; a kinematic body's `mass` is written only where the node holds a value |
| `initial_linear_velocity`, `initial_angular_velocity` | `physics:velocity`, `physics:angularVelocity` | body space on both sides; as with the mass properties, a static body's local velocities travel as `erhe:Node_physics:<name>` |
...
| mesh collision shape (`Physics_node_geometry::mesh`, `convex_hull`) | `PhysicsCollisionAPI` on the `Mesh` prim itself, `PhysicsMeshCollisionAPI` `physics:approximation = convexHull` or `none` | a dynamic body's mesh shape is a convex hull on both sides (Jolt); `boundingCube` / `boundingSphere` / `convexDecomposition` / `meshSimplification` read as `convexHull` with one warning. The shape names the mesh it was built from (`Node_physics.collision_mesh`, no value = the body's own mesh), so a body whose mesh is a prim below it - the `Rock/shell` form - writes the schemas on that prim and keeps the motion on the body prim, and a reload finds the geometry again |
...
| `Physics_joint_settings` (a `Typed` prim: eleven per-axis properties of each of the six degrees of freedom) | typeless prim where it sits with `PhysicsLimitAPI:<axis>` (`physics:low`, `physics:high`) and `PhysicsDriveAPI:<axis>` (`physics:type`, `physics:maxForce`, `physics:targetPosition`, `physics:targetVelocity`, `physics:stiffness`, `physics:damping`) applied per axis; a limit's `stiffness` and `damping` as `erhe:limit:<axis>:stiffness` / `erhe:limit:<axis>:damping` | axis instance names `transX transY transZ rotX rotY rotZ`, one instance per limited or driven axis; those attributes are the native USD form of all 66 properties, so none of them is repeated as an `erhe:Physics_joint_settings:` attribute, and the reader sets the properties of the axis each instance names |
| `editor::Joint` prim (`body_0`, `body_1`, `joint_settings`, `enable_collision`), which sits below the prim `body_0` names | child `PhysicsJoint` prim of that node named after the settings item: `physics:body0` the nearest prim at or above `body_0` that carries a body (`body_0` itself when none does), `physics:localPos0` / `localRot0` `body_0`'s transform in that prim's space, `physics:body1` and `physics:localPos1` / `localRot1` the same for `body_1`, `physics:collisionEnabled = enable_collision`, the settings' `LimitAPI` / `DriveAPI` instances applied inline, and `rel erhe:Joint:joint_settings = </path of the settings prim>` | the erhe six-dof joint is a plain `PhysicsJoint`; the reader shares a settings item through the relationship, else makes one from the inline instances; `PhysicsRevoluteJoint` / `PhysicsPrismaticJoint` read as one rotational / one linear limit on `physics:axis` (`lowerLimit`, `upperLimit`), `PhysicsFixedJoint` as six limits at zero, `PhysicsSphericalJoint` as the cone angles on the two axes beside `physics:axis`, `PhysicsDistanceJoint` as one linear limit over all three axes (`minDistance`, `maxDistance`); a non-identity frame is read as an `Xform` node named `<joint prim name>_frame0` / `_frame1` below the body prim - the joint prim sits below the first and names the second - and a prim of that name already carrying that transform is that node, so a reload of erhe's own output makes no second one. The joint prim itself is not written as a prim of its own: it IS the `PhysicsJoint` prim, and the reader places it below the first frame node, named after the `PhysicsJoint` prim |
...
| none | `specializes` (`S`) | |
| a scene's whole content | a root layer's `subLayers` (`L`) | composed at load, strongest first: the root layer's opinions beat every sublayer, an earlier `subLayers` entry beats a later one, a prim absent from the stronger layers is added whole, and stage metadata the root leaves unauthored comes from the strongest sublayer that authors it. A save writes ONE layer holding the composed content and authors no `subLayers` - erhe edits the flattened stage and has no layer to write an edit back to (`doc/erhe/usd.md`, "Sublayers"); a stack the editor could edit layer by layer is future work (`doc/plans/usd_compatibility.md`, "Layer-stack editing") |
| none | session layer | an undo stack is not a layer |
| `Value_source` | opinion provenance | erhe resolves every arc itself, so the erhe value source IS the composition provenance; the table below restates each source as the USD origin the Properties window and MCP report (`doc/erhe/usd_compatibility_design.md` X5) |

### Where a value comes from

`editor::describe_property_origin` derives this on demand from the item, its
scene's file, the arc carrier above it and the `Style` it uses;
