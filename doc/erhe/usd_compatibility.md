...
| `Xform` with several applied-API-schema attachments (`Node_physics`, `Node_joint`, `Brush_placement`, `Prefab_instance`, `Frame_controller`, `Grid`) | `Xform` with those schemas applied to it | what USD applies to a prim as an API schema is what erhe attaches to a node |
...
| `Layout` (a value group of the node, `doc/erhe/property_system.md` section 4.13), `Brush_placement`, `Grid`, `Rendertarget_mesh` | custom (codeless) schemas or namespaced custom attributes (`erhe:...`) | editor domain, no USD counterpart; the attribute form is the qualified-name row of "Property system" |
