...
| `Xform` with several applied-API-schema attachments (`Node_joint`, `Prefab_instance`) | `Xform` with those schemas applied to it | what USD applies to a prim as an API schema is what erhe attaches to a node |
...
| the node's `Node_physics.*` values / a `Node_joint` attachment | `UsdPhysics` API schemas / joint prims, see "Physics" | |
...
| `Node_joint` on a prim (`connected_node`, `joint_settings`, `enable_collision`) | child `PhysicsJoint` prim of the node named after the settings item: `physics:body0` the nearest prim at or above the node that carries a body (the node itself when none does), `physics:localPos0` / `localRot0` the node's transform in that prim's space, `physics:body1` and `physics:localPos1` / `localRot1` the same for the connected node, `physics:collisionEnabled = enable_collision`, the settings' `LimitAPI` / `DriveAPI` instances applied inline, and `rel erhe:Node_joint:joint_settings = </path of the settings prim>` | the erhe six-dof joint is a plain `PhysicsJoint`; the reader shares a settings item through the relationship, else makes one from the inline instances; `PhysicsRevoluteJoint` / `PhysicsPrismaticJoint` read as one rotational / one linear limit on `physics:axis` (`lowerLimit`, `upperLimit`), `PhysicsFixedJoint` as six limits at zero, `PhysicsSphericalJoint` as the cone angles on the two axes beside `physics:axis`, `PhysicsDistanceJoint` as one linear limit over all three axes (`minDistance`, `maxDistance`); a non-identity frame is read as an `Xform` node named `<joint prim name>_frame0` / `_frame1` below the body prim - the joint sits on the first and names the second - and a prim of that name already carrying that transform is that node, so a reload of erhe's own output makes no second one |
