...
verification workflow. `doc/erhe/property.md` is the library
...
`doc/erhe/property_inventory.md` is the status table: every registered
...
commit named in `doc/reference/property_system_wpf_comparison.md`, files under
...
- D8 Inheritance mechanics. `Dependency_object` has
  ...
  (`doc/editor/content_library_folders.md` D1). An inherits-flagged property
...
- D25 Style layer (WPF `Style` setters, `BaseValueSourceInternal.Style`).
  - Context. The graphics presets are a generated POD
    ...
    library's style items (`doc/editor/style_library.md`), which any item,
  ...
  - Library. A style source is a `Dependency_object` whose LOCAL values
    ...
    style items are their own sources (`doc/editor/style_library.md` D2). R3 is
  ...
  - Serialization. A style item and the style each material and library
    folder uses are saved by name (`doc/editor/style_library.md` D4); a source
  ...
  - Editor. Every item has a `style` property, a bridged object reference
    over the source (`Item_base::style_property`, `doc/editor/style_library.md`
...
- D28 Object references. `Object_reference` (the last `Property_value`
  ...
  - Text form. `to_string` is the pointee's `get_reference_path()`, a
    ...
    `doc/erhe/usd_compatibility_design.md` M1; `Item_base`: the item name; empty
...
- D30 Secondary owner type. `Dependency_object::
  ...
  (`doc/editor/content_library_folders.md` D8): a Materials folder holds
...
- D32 A local value is an authored value. `Value_source::local` means
  ...
  (`doc/erhe/usd_compatibility_design.md` M4). An importer that fills an item
  ...
  `doc/erhe/property_inventory.md` owns the list of registrations that carry
  it.
- D33 Reference layer (a USD reference arc, `doc/erhe/usd_compatibility_design.md`
  X2).
...
`doc/erhe/property_inventory.md`, updated together with these subsections.
...
`doc/erhe/scene.md` "Animation playback".
...
- Command scripts: `scene.set_property` (`config/editor/commands.json`,
  `doc/editor/command_script.md`) with `item`, `property` and `value` or
...
already hit. `doc/erhe/property_inventory.md` owns the per-field status and
...
- Update `doc/erhe/property_inventory.md` (storage kind `entry`, `inherits`)
  and the owner's subsection in `doc/erhe/property_system.md` in the same
...
[0, pi]), `stiffness` (vec3, coerced to [0, 0.99]; read by the
constrained IK solve), `rest_rotation` (quat),
