...
`doc/erhe/property_system.md`; the status table of every registered property
by owner with its storage kind, and of the item fields not yet migrated,
is `doc/erhe/property_inventory.md`. A change to a mechanism here updates this
...
- **`Property_value`** - `std::variant<bool, int, float, glm::vec2, glm::vec3,
  ...
  std::vector<float>, std::vector<int>, Weak_object_reference,
  ...
  (`Property_type::asset_path`, text `asset`) and the arrays
  (`Property_type::float_array` / `int_array` / `string_array`, text
  `float[]` / `int[]` / `string[]`, `doc/erhe/property_system.md` D35)
  are the USD value types (`doc/erhe/usd_compatibility_design.md` M6): a `double`
  ...
  row is the value count and the first few values plus one editor per
  element (D34), with `Property_ui::array_size` deciding whether the row
  also offers add and remove. A `string_array` element is double quoted in
  the D16 text, so it may hold spaces, quotes or be empty. The
...
- **`Property_bridge`** - `get` / `set` callbacks that store a property in
  ...
  `doc/erhe/property_system.md` section 4.5).
...
- **`Expression`** (`expression.hpp`) - a compiled formula driving one
  property (`doc/erhe/property_system.md` D22): comma-separated tinyexpr
...
(`doc/erhe/property_system.md` D5) hold a playback pose above the local layer
...
(`doc/editor/style_library.md`) are sources of their own and are the ones a scene
saves.
...
inheritance descendants to read (`doc/erhe/property_system.md` D30).
...
only, read by serializers, listed in `doc/erhe/property_inventory.md`.
