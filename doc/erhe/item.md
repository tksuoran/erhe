...
- **`Item_base`** - Base class for all items. Provides ID, name, flags, tags, source path, debug label. Inherits `enable_shared_from_this` - all instances must be created via `std::make_shared`. Also derives from `erhe::property::Dependency_object` (see `doc/erhe/property.md`): every item carries a property store, and `get_property_owner_type()` returns `get_type()` so property metadata resolves by item type.
...
- **`Typed`** - A typed prim (`doc/erhe/usd_compatibility_design.md` C5): the level of the prim class hierarchy that carries the USD `typeName` token, see "Prim classes".
...
- `get_reference_path()` / `get_shared_reference()` - the text and `shared_from_this` an object reference (`doc/erhe/property_system.md` D28) uses to name and hold this item. `Item_base` names an item by its name; `Hierarchy` overrides it with the item's path (see "Item paths")
...
(`doc/erhe/usd_compatibility_design.md` C5). A level's class has its own
...
(`doc/erhe/property_system.md` D27) follows the same levels. `erhe::item` holds
...
(`doc/erhe/property_system.md` D18) over `get_prim_type_name()` /
...
(`doc/editor/content_library_folders.md` D1).
...
in `erhe::scene`, see `doc/erhe/scene.md` "Prim levels".

`Scope::get_secondary_property_owner_type()` is the root owner type, as an
editor `Style` item's is (`doc/erhe/property_system.md` D30), so a scope holds
...
(`doc/erhe/usd_compatibility_design.md` M2), so an item path names exactly one item
...
| `test_item_active.cpp` | 10 | Derived `active` bit follows the local value, an ancestor's value over the whole subtree, an item's own false value under an active ancestor and a tree move; `set_flag_bits` drops the bit; a copy re-derives from its own value; a pruning parent clears its children's subtrees while keeping its own bit, both opinions are required, and the pruning follows a tree move |
