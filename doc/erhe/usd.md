...
- The converter runs with triangulation and vertex-index building off. That
  ...
  needs; the element table of `doc/erhe/usd_compatibility.md` maps the
...
- Every prim the conversion gives a transform to keeps the xformOp stack it
  ...
  (`doc/erhe/usd_compatibility_design.md` M8, `doc/erhe/scene.md` "Authored
...
- The erhe class of a prim is the class its `typeName` names
  (`doc/erhe/usd_compatibility_design.md` C5, the object-model table of
  `doc/erhe/usd_compatibility.md`): a `Mesh` prim becomes an
...
- A `Material` prim becomes the erhe material prim, parented where the
  stage puts it (`doc/erhe/usd_compatibility_design.md` U4): a stage keeping its
...
- Values are read at the stage's default time code, and the `UsdPhysics`
  ...
  below).
- A local value is an authored value (`doc/erhe/property_system.md` D32). Tydra
  ...
  `diffuseColor` falls back to 0.18); `doc/erhe/usd_compatibility.md` states that
  per row.
...
default (`doc/erhe/usd_compatibility_design.md` I2). Of the inputs erhe carries only
...
`doc/erhe/scene_renderer.md`). A mesh's `primvars:displayColor` arrives
...
`doc/erhe/primitive.md`). Nothing of that input is then written as a
...
importer wants (doc/erhe/usd_compatibility_design.md X1): a prim that authors arcs
...
- The TARGET of an arc is any prim: an `Xformable`, a `Scope`, the `Typed`
  ...
  wrapper reproduces (`doc/editor/parsers.md`).
...
- An `over` prim the referencing layer authors below a referencing prim is
  the sparse override of one instance item (doc/erhe/usd_compatibility_design.md X2),
...
(`doc/editor/style_library.md` D25). Tydra's render-scene conversion reports a class
...
themselves (doc/erhe/usd_compatibility_design.md X3).
...
(doc/erhe/usd_compatibility_design.md S1). The prim itself is an
...
(doc/erhe/usd_compatibility_design.md E4a): `def Brush "<name>"` where the brush sits
...
(doc/erhe/usd_node_graphs.md). A `NodeGraph` without it is a foreign
...
(doc/erhe/usd_compatibility_design.md X4). Material bindings, property opinions and
...
load returns (doc/erhe/usd_compatibility_design.md C6), so a path this tree cannot
...
`inverse(bind_j) * geomBindTransform` (doc/erhe/usd_compatibility_design.md K1), so
...
- The transform the writer writes for a prim is the one the prim authored -
  ...
  animated layer (`doc/erhe/property_system.md` D5) - so a save made while an
...
layer (`doc/erhe/property_system.md` D5) while the keys hold the edit, so a save
...
previous one (`doc/erhe/scene.md`, "Animation playback").
...
is the "Physics" table of `doc/erhe/usd_compatibility.md`; the rules the read
follows are:
...
mapping is the "Draw modes" table of `doc/erhe/usd_compatibility.md`. Every
...
- Prim layout. The root node is not a prim: an erhe item path excludes the
  ...
  tree survives a save (doc/erhe/usd_compatibility_design.md E4d,
  doc/editor/content_library_folders.md). A skin and an animation are library
...
- Prim `active` metadata (`doc/erhe/usd_compatibility_design.md` X2). The item's
...
- The name a value is authored under. `native_usd_property_name` is the one
  ...
  (`doc/erhe/usd_compatibility_design.md` X5); `Native_property_form` says which of
...
- Styles and inherits arcs (doc/erhe/usd_compatibility_design.md X3). A style item is
...
- Brushes (doc/erhe/usd_compatibility_design.md E4a). A brush item is written as the
...
- Composition arcs. A prim the caller names in
  ...
  (doc/erhe/usd_compatibility_design.md X1). An arc's `asset_path` is
...
- What a carrier does write of the instance below it is the overrides its
  items hold (doc/erhe/usd_compatibility_design.md X2), collected through
...
(doc/editor/scene_serialization.md, "Verifying round-trips").
...
- lives in `src/editor/parsers/usd.{hpp,cpp}`; see `doc/editor/parsers.md`.
