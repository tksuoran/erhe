...
> **Status (2026-05-22): largely landed.** Vulkan + OpenXR runs on Quest 3
> (see the OpenXR session bring-up referenced in `doc/quest.md` Phase 5
> and `doc/multiview.md`). The function-pointer loading, session
> creation, swapchain enumeration, and `Render_view` Vulkan paths
> described below are wired in `src/erhe/xr/erhe_xr/xr_instance.{hpp,cpp}`
> and `xr_session.{hpp,cpp}`. Treat this document as historical context
> for the original plan; cross-check against the current source before
> citing specific symbol or line numbers.
