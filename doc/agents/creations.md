...
documented in `doc/agents/editor_runs.md` ("In-editor MCP server") and
...
| Creation feature | Editor feature underneath |
|---|---|
| `save_scene` / `load_scene` (`res/editor/scenes/creations/*.glb`) | The glTF exporter/importer with erhe extensions: `ERHE_brushes` (content-library brushes), `ERHE_node` (rigid-body values), `ERHE_scene` (scene settings, physics materials) - a saved creation round-trips with its physics rig and brush pool intact |
...
- `doc/agents/editor_runs.md` "In-editor MCP server" + `doc/agents/mcp_server_usage.md` -
