...
`Items_removed_message` (doc/import_undo_reference_clearing.md).
...
- Mesh VRAM returns to the pool free list, not to the driver. `Buffer_pool`
  ...
  of growing new blocks (doc/mesh_memory.md).
- `materials_as_references = true` leaves a second container parse in the
  asset manager, untouched by the drop (doc/asset_manager.md, "Current
  restrictions").
