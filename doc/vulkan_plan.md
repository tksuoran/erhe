...
| 12 | 2026-04-04 | example | VUID-VkGraphicsPipelineCreateInfo-layout-07990 | Sampler/buffer binding namespace collision: non-bindless sampler at binding 0-1 collided with UBO/SSBO at same indices. Fix: enable `use_bindless_texture=true` for Vulkan (uses descriptor indexing via texture heap at set 1), eliminating non-bindless sampler declarations from set 0. Updated Texture_heap to accept Bind_group_layout for pipeline layout compatibility. | `vulkan_device.cpp`, `shader_stages_create_info.cpp`, `texture_heap.*`, all renderers |
