...
- D2 Value representation.
  ...
  types of `doc/usd_compatibility_design.md` M6 - `Property_type::double_floating`,
...
- D8 Inheritance mechanics. `Dependency_object` has
  ...
  (`doc/content_library_folders.md` D1). An inherits-flagged property
...
- D25 Style layer (WPF `Style` setters, `BaseValueSourceInternal.Style`).
  - Context. The graphics presets are a generated POD
    ...
    library's style items (`doc/style_library.md`), which any item,
  ...
  - Library. A style source is a `Dependency_object` whose LOCAL values
    ...
    style items are their own sources (`doc/style_library.md` D2). R3 is
  ...
  - Serialization. A style item and the style each material and library
    folder uses are saved by name (`doc/style_library.md` D4); a source
  ...
  - Editor. Every item has a `style` property, a bridged object reference
    over the source (`Item_base::style_property`, `doc/style_library.md`
...
- D28 Object references. `Object_reference` (the last `Property_value`
  ...
  - Text form. `to_string` is the pointee's `get_reference_path()`, a
    ...
    `doc/usd_compatibility_design.md` M1; `Item_base`: the item name; empty
...
- D30 Secondary owner type. `Dependency_object::
  ...
  (`doc/content_library_folders.md` D8): a Materials folder holds
  ...
  under `Xformable` (`doc/usd_compatibility_design.md` C5) while `Light` and
...
- D31 Per-object default. `Property_metadata::compute_default`, when
  ...
  `Item_base::purpose_property` (`doc/usd_compatibility_design.md` M3),
...
- D32 A local value is an authored value. `Value_source::local` means
  ...
  (`doc/usd_compatibility_design.md` M4). An importer that fills an item
  ...
  `doc/property_inventory.md` owns the list of registrations that carry
  it.
- D33 Reference layer (a USD reference arc, `doc/usd_compatibility_design.md`
  X2).
...
`doc/property_inventory.md`, updated together with these subsections.
...
(`Item_base::style_applies`, `doc/style_library.md` D3). The
...
way and so holds every class's values (`doc/style_library.md` D2). `ERHE_node` carries the held values in its `properties`
...
already hit. `doc/property_inventory.md` owns the per-field status and
...
- Update `doc/property_inventory.md` (storage kind `entry`, `inherits`)
  and the owner's subsection in `doc/property_system.md` in the same
