...
The normative, observable behavior of the frame pacer in each scenario the requirements ([requirements.md](requirements.md)) name. Mechanisms are specified in [algorithm.md](algorithm.md); the control-theoretic grounding is [control_model.md](control_model.md). Numeric bounds use the default tunables (60 Hz examples, `T` = 16.67 ms); "measured" values are from the verification simulation (`scripts/frame_pacing_sim.py`), cited by claim number.

Each scenario gives: the trigger, the required behavior (*shall*-statements - these are the acceptance contract, asserted by the parity suite `erhe_frame_pacing_tests`), quantitative bounds, and the **record signature**: what the scenario looks like in the frame-record data, so profiling tools and humans can recognize it. The editor's verification UI ([user_interface.md](user_interface.md)) is built to make these signatures readable by eye.
...
- Believed self-miss rate shall not exceed the AIMD equilibrium `delta_dec/Delta_inc` (default 1 %); actual slot misses shall be rarer (the `guard`-wide near-miss band absorbs the tail). Measured: 0 misses in 960 frames (C1).
- Release times shall sit at `d_k - (L_hat_p + m)`; average scheduling slack ~= `m`, which saw-tooths in `[m_min, m_min + Delta_inc]` with decay period `Delta_inc/delta_dec` = 100 frames after each rare near-miss.
...
**Trigger:** the p-quantile of either stage rises above `N*T - guard` (ramp) or frames start missing outright (abrupt step). CPU and GPU steps are handled identically - the decision statistic is `max(c_hat_p, g_hat_p)` and the streak attribution uses per-frame `max(c, g)`.
...
- Disturbed frames = the spike itself plus the GPU-queue cascade, ~= `ceil((S - budget)/(N*T - g)) + 1` frames for spike size `S`, plus at most one slipped frame during re-anchoring (measured 3 total).
- The margin shall take one attack step per disturbed self-missing frame and decay back to its pre-spike level within `Delta_inc/delta_dec` frames per step; no permanent latency cost (C5).
...
- A single loaded frame during the dwell shall reset it (conservative by design; borderline load therefore never upshifts - see section 5).
- The transition shall be miss-free: on upshift the new cadence's latency estimate is seeded with `max(carry-over, c_hat_p + g_hat_p)` and the margin gets a pre-emptive `Delta_inc` boost (measured 0 transition misses, C3).
...
**Trigger:** the resolved tier is OFF (see [capability_tiers.md](capability_tiers.md) section 2). The scenarios above describe tier W; tier S paces from backpressure alone and has no cadence, no grid and no display-time sensing, so only the cross-scenario invariants marked "Always" apply to it.

- `schedule()` shall return release = now, no target present time, no wait ids; no pacing decisions of any kind are made and no claims from section 1-section 9 apply.
