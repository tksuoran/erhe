...
- `cpu_dur` and `gpu_dur` are stage **service times** — involuntary waits (pacer wait, fence, acquire) and GPU idle bubbles excluded per [frame_pacing_inputs.md](inputs.md) §3.2/§3.3. Raw spans contaminate the outer-loop statistic `W` with pacing-induced wait time and can cause spurious downshifts.
