...
| Process dies inside `vkQueuePresentKHR` when `VkPresentTimingsInfoEXT` is chained | A swapchain that receives timing requests must be created with `VK_SWAPCHAIN_CREATE_PRESENT_TIMING_BIT_EXT` (`VUID-vkSetSwapchainPresentTimingQueueSizeEXT-swapchain-12229`); without the flag the driver crashes silently, and only the validation layer names the cause | Keep erhe free of Vulkan validation errors and run validation when touching Vulkan code (a doc/erhe/gpu_coding_rules.md rule motivated by this trap). With the flag set, requests and feedback work (feedback `complete=true`, achieved-present ~2 ms after GPU end at 120 Hz) |
