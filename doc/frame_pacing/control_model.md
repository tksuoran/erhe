...
The model of the frame pacing problem in the framework of control theory. It defines the plant, signals, disturbances and controller structure, states the stability and convergence properties the algorithm has, and lists the claims verified in simulation. The algorithm itself is [algorithm.md](algorithm.md), which builds on this model and records where it refines it.
...
```
                ...
                |  +-----+------------+           |      | time (FR3)        |
                |  | Estimators:      |-----------+      v                   |
                |  | vsync grid V_hat,T_hat  |         +------------------+         |
                |  | stage times c_hat,g_hat  |         | Queue clamp      |         |
                |  | latency L_hat, delta_hat_pe |         | (FR5, hard)      |         |
                |  +------------------+         +------------------+         |
                |        ^                                                   |
                +--------+---------------------------------------------------+
                         |
...
A structural property worth stating up front: releases are **time-triggered, anchored to the vsync grid**, not completion-triggered (frame k+1 does not start "when frame k is done" but "at the scheduled time for slot j_{k+1}"). This breaks the positive-feedback path where one late frame delays the next release, which delays the next, producing a death spiral. Self-timed pipelines are marginally stable at best; grid-anchored pipelines reject one-frame disturbances structurally (section 7.4).

## 2. Notation and signals

| Symbol | Meaning |
|---|---|
| `T` | Display refresh period (fixed; slow drift tracked, see section 5.1) |
| `V_j = V_0 + j*T` | Vsync (scanout) times, `j` integer; `V_hat` denotes the estimated grid |
| `k` | Frame index |
| `N_k in {N_min, N_min+1, ...}` | Vsyncs per frame (outer loop output); `N_min` is the application-set minimum |
| `j_k` | Target vsync slot for frame k; in steady state `j_{k+1} = j_k + N_k` |
| `delta_pe` | Presentation-engine deadline offset: how long before `V_j` the image must be complete to be presented at `V_j` (platform/path dependent, estimated) |
| `d_k = V_hat_{j_k} - delta_hat_pe` | Completion deadline for frame k |
...
| `x_hat_p` | Estimated p-quantile of random quantity `x` (e.g. `c_hat_p`, `g_hat_p`, `L_hat_p`) |
...
|---|---|---|
| Release gating (wake app at `r_k`) | Inner | OS timer granularity + scheduler jitter, ~0.5-1 ms worst case on Windows with high-resolution timers; folded into the margin (section 6.2) |
...
| FR1 (no judder/oscillation) | No limit cycles in the switching loop: the closed loop under stationary load must converge to a constant `N` in finite time and stay there. 1-2-1-2 cadence alternation *is* a period-2 limit cycle of the outer loop and is excluded by design (section 7.2). |
| FR5 (queue depth) | Invariant: `q_k <= Q*` + bounded transient, guaranteed unconditionally (hard clamp), not merely in expectation. |
| FR4 (predicted display time) | Observer accuracy: predicted `(V_hat_{j_k}, N_k*T_hat)` equals the realized display time and duration whenever the chance constraint holds - i.e. FR4 is a byproduct of FR2a plus grid estimation, not a separate loop. |
...
The asymmetry demanded by the requirements - downshift fast, upshift deliberately - is the classic *fast-attack, slow-release* policy and appears in three places: the reactive downshift path (section 6.3), the margin adaptation (section 6.2), and the upshift dwell time.

## 5. Disturbance model

The design is analyzed against, and the simulator generates, the following disturbance classes:
...
| Impulse | Single-frame spike in `c` or `g` | At most one late frame; must *not* change cadence (section 7.3) |
...
| Periodic | Background task beating against frame cadence | Absorbed if within quantile; else appears as borderline load (section 7.2) |
| Structural | Missed present, cold start, swapchain recreation | FR7 recovery paths (section 8) |

Measurement disturbances: presentation feedback delay (section 3.5), GPU-timestamp calibration error (small, absorbed into margin), and censoring of display outcomes (section 3.2).

## 6. Controller structure

### 6.1 Estimators (shared)

- **Vsync grid tracker**: estimates `V_hat_0, T_hat` from achieved present times - structurally a phase-locked loop with a low-gain integrator; `T` is nominally known from the display mode, so the tracker mainly maintains phase. Its bandwidth is far below all other loops and it can be treated as a static reference in their analysis.
- **Stage-quantile estimators** `c_hat_p`, `g_hat_p`, `L_hat_p`: online quantile trackers of the form `x_hat <- x_hat + eta*(p - 1{x <= x_hat})` (stochastic gradient on the pinball loss). By stochastic-approximation theory this converges to the true p-quantile for stationary input; with a constant gain `eta` (required, to track change points) the estimate has bounded steady-state ripple proportional to `eta`. **Design constraint:** the ripple of the decision statistic must be smaller than the outer loop's hysteresis width `H` (section 7.2). The frame-record history (default 120 frames) bounds the data window; the estimator gain, not the window, sets the effective time constant.
- **`delta_hat_pe` estimator**: per presentation path (fullscreen / windowed), from correlating GPU-end times with hit/miss outcomes and achieved present times. Slowly varying; conservative (over-)estimate preferred, error is absorbed by the margin.
...
```
j_k  = j_{k-1} + N_{k-1}                      (next slot per current cadence, re-anchored on feedback, section 3.3)
d_k  = V_hat(j_k) - delta_hat_pe
r_k  = clamp( d_k - L_hat_p - m_k ,  lower: now,  upper: d_k - L_floor )
        ...and additionally gated by the present-wait clamp (section 3.3)
```

Because `s_k = d_k - r_k - L_k = (L_hat_p + m_k) - L_k`, the plant from `r` to `s` is a **unit gain with additive noise**: there are no dynamics to compensate, and closed-loop behavior is entirely determined by the estimator `L_hat_p` and the margin `m`. This reframes FR2 as a *quantile prediction problem with a safety allowance*, which is the central modeling insight of the inner loop.

Role separation, to prevent the two adaptive elements from fighting:

- `L_hat_p` tracks the p-quantile of *measured* latency - fast timescale (per frame).
- `m_k` covers what the estimator cannot: estimation ripple, actuation jitter (section 3.4), `delta_hat_pe` error, non-stationarity between change points - slow timescale, adapted on *realized outcomes*: additive increase on a miss or near-miss (fast attack), slow decay toward `m_min` when slack is consistently ample (slow release). This is AIMD-shaped, prioritizing the constraint (FR2a) over the performance objective (FR2b) exactly as required. `m` is clamped to `[m_min, m_max]`; `m_0` is the cold-start tunable (20 % of `T`).
...
Decision statistic: `W_k = max(c_hat_p, g_hat_p) + guard`, the throughput requirement per section 3.1 (`guard` covers per-frame actuation/scheduling jitter). Feasibility of cadence `N`: `fit(N) == W_k <= N*T`.

Switching law - a hysteretic relay with dwell time and an event-triggered override:

- **Predictive downshift**: if `not fit(N_k)`, set `N <- min{N >= N_min : fit(N)}` immediately. (Estimator-driven; catches sustained load growth before misses accumulate.)
...
With the plant being unit-gain (section 6.2), inner-loop stability reduces to estimator convergence:

- The quantile tracker is a Robbins-Monro recursion; for stationary `L` it converges to the p-quantile with steady-state ripple `O(eta)`. No gain choice can make it unstable in the control sense (the recursion is a contraction in expectation for any `0 < eta`, since the pinball subgradient is bounded); the trade-off is purely ripple vs. tracking speed.
...
1. **Hysteresis:** upshift and downshift thresholds are separated by `H`. A limit cycle requires `W` to traverse the full band; steady-state estimator ripple is designed `< H` (section 6.1), so stationary load cannot cause traversal. This yields the design rule tying `eta` (estimator gain) to `H`.
...
| One-frame spike | One late frame; no cadence change | (C5, section 7.3) |
...
| Swapchain recreation | Grid/`delta_hat_pe`/queue state reset; **stage-time estimates retained** (load did not change); re-converges like a cold start with warm load estimates | << cold start (C7) |
...
- **Fullscreen / independent flip:** `delta_pe` is small; feedback reflects actual scanout; the model applies directly.
- **Windowed (DWM):** the compositor is an additional sampled stage running on its own vsync - modeled as a larger, more variable `delta_pe` plus one added period of display latency; presentation feedback may reflect composition rather than scanout. All of this is absorbed by the `delta_hat_pe` estimator and the margin; no structural change to the loops. The achievable pacing quality is lower (larger effective deadline offset and feedback uncertainty); it is quantified per capability tier in [capability_tiers.md](capability_tiers.md) section 4.

The pacer detects the active path (where the platform reports it) or simply inherits it through the estimated `delta_hat_pe`; the control laws do not branch on it.
...
5. Cross-frame coupling through shared resources (fence waits, GPU contention) is captured *empirically* inside measured `L_k`, not structurally. This is adequate because the inner loop predicts `L` as observed, but it means `L_hat_p` is learned per cadence `N`: the algorithm keeps one latency window per cadence and seeds a new one from the old cadence's quantile, because waits change when the period changes.
...
These are the claims a deterministic simulation (virtual clock, synthetic `c/g` processes, modeled presentation engine per section 3.2) demonstrates. `scripts/frame_pacing_sim.py` is that simulation and the executable reference model; `erhe_frame_pacing_tests` is its C++ parity suite, and both run each claim by number. Claims C12 and beyond were added as the integration met regimes this section did not anticipate (dynamic grid, dropping presentation paths, inert present-time targets, grossly wrong queried refresh periods, scanout skips, the inline holdback, the tier S servo); each is stated with its scenario and result in [algorithm.md](algorithm.md) section 8.
...
- **C5 - Impulse:** single-frame spike -> spike + GPU-queue cascade drain (section 7.3) + <= 1 re-anchoring slip only, no cadence change, margin returns to pre-spike level.
- **C6 - Queue invariant:** under every scenario above plus adversarial estimate corruption (force `L_hat` to zero), `q <= Q* + F` always; `q` returns to `Q*` within one re-anchor after a miss.
...
- **C8 - Gain/hysteresis margin:** measured steady-state ripple of `W` < `H` with the chosen `eta`; report the ratio (design headroom). Additionally sweep gains toward each other to locate the timescale-separation limit (section 6.4) and confirm the defaults sit well inside it.
...
|---|---|
| Downshift reaction time (<= 2 frames) | `M`, reactive downshift threshold (section 6.3) |
| Upshift confirmation time (500 ms) | `T_up`, dwell time (section 6.3) |
| Convergence after disturbance (<= 1 s) | Settling-time bound in C2/C7; constrains estimator gain `eta` from below |
| Latency safety margin initial (20 % of `T`) | `m_0`; `m in [m_min, m_max]` adaptive (section 6.2) |
| Minimum vsyncs per frame | `N_min` (section 6.3) |
| Target queued present images (1) | `Q*` (section 3.3) |
| Frame record history (120) | Data window bounding all estimators (section 6.1) |
| - (new, introduced by the model) | `p` target quantile; `H` hysteresis width; `eta` estimator gain; `guard` jitter allowance; `K` cold-start seed frames; `m_min`, `m_max` |

The parameters introduced by the model (bottom row) are part of the algorithm's tunable set, with defaults justified against the claims; the consolidated set is [algorithm.md](algorithm.md) section 7.
