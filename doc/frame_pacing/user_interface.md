# Frame Pacer - Verification UI and Simulated Workload

Stability: mostly stable

The editor's "Frame Pacing" ImGui window (`editor::Frame_pacing_window`), which
lets a human visually verify frame pacer behavior on real hardware, and the
adjustable simulated workload that drives the pacer through its scenarios. The
`U` labels below are cited from the window's source and keep their numbering.
The window is collected at the editor tick right after the pacing observer;
its headless twins are the MCP tools `get_frame_pacing_status`,
`get_frame_pacing_frames` and `set_frame_pacing_workload`.

Related documents: [requirements](requirements.md) (Data requirements section
defines the frame records this UI consumes), [algorithm](algorithm.md)
(N, T, Q*, F, release/deadline terminology), [behavior](behavior.md)
...
- The wait shall be precise: actual duration within the jitter guard (~1 ms)
  ...
  this (measured p99 wake error 15.3 ms); a high-resolution waitable
...
DYNAMIC vsync grid (tracked phase + tracked period,
[algorithm.md](algorithm.md) deviation 10), captured per frame and applied sample-locally, never from
...
instrumentation path ([algorithm.md](algorithm.md) section 9) - and copies them into a
...
- [../plans/frame_pacing.md](../plans/frame_pacing.md) - simulated GPU workload knob, capture serialization.
