...
Grouped by production site. "Consumer" names the pacer interface call ([frame_pacing_algorithm.md §1](algorithm.md)) that ingests the input. "Latency" is when the input becomes causally available relative to the frame it describes — the design tolerates every latency listed (verified by the simulation's event-delivery model).
...
Two CPU-timeline observations come for free, need no clock calibration, and serve as cross-checks on the primary sources. They shall be recorded and compared, feeding the degradation diagnostics of [frame_pacing_capability_tiers.md](capability_tiers.md) §5; they are never fed to the control loops.
