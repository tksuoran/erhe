...
| none (plan X2: `Item_base::active`, default true; false removes the item and its subtree from every consumer) | `active` prim metadata | the way a prim inside a reference is taken out; an `over` carries it |
