...
| content library (per-scene index of the scene's resources) | the prim tree itself | a resource is a prim where it sits; the library only indexes them (`src/editor/content_library/notes.md`). A folder is a `Scope`, and each kind has one `Scope` below the scene root (`/Materials`, `/Brushes`, ...) - where a resource goes when nothing else places it. glTF carries the position in `ERHE_scene` `library_folders` (`doc/gltf_extensions/ERHE_scene.md`), where a path names any prim of the tree; in USD the tree is the file, and the `Material` prim row of "Object model" says how far that has come |
...
| `Scope` (children only, no transform) | `Scope` | a transform composes through it to the nearest transformable ancestor. A content-library folder is one, and so is each kind's scope (`/Materials`, `/Brushes`, ...); a `Scope` a USD file authors round-trips as itself, except the scope that gathers a stage's `Material` prims, which is still read as the namespace of a material library rather than as a prim of the tree - see the `Material` prim row |
...
| `erhe::primitive::Material` (a `Typed` prim) | `Material` prim (`UsdShadeMaterial`) with its `UsdPreviewSurface` / `UsdUVTexture` shader network as child prims | a material is a prim of the erhe tree wherever the user puts it, and glTF carries that. USD does not yet: the writer gathers a scene's materials under one `/Materials` scope built from the library index, and the importer reads a stage's material scope as that namespace rather than as a `Scope` prim holding `Material` prims. Writing a `Material` prim where it sits, and reading one back in place, is the remaining USD step (`doc/usd-compatibility-plan.md`) |
