...
source comments cite (`doc/gltf_scene_roundtrip.md phase 3`). The pipeline as
it runs, and the list of what is and is not persisted, is
`doc/scene_serialization.md`; the wire format of each extension is
`doc/gltf_extensions/`.
...
Khronos context; the USD naming mapping is in `doc/usd_compatibility.md`.
...
concept mapping lives in `doc/usd_compatibility.md`, and the steps toward USD
interchange and composition in `doc/usd_compatibility_design.md`. erhe
...
`doc/usd_compatibility_design.md` step X2, rather than by an `ERHE_overrides`
...
`doc/scene_serialization.md` "Verifying round-trips". Its sections are:
...
Known non-round-tripping state is listed in `doc/scene_serialization.md`
...
"USD-inspired features" and `doc/usd_compatibility.md`): collections shape
`ERHE_collections`, and overs, variants and animate-anything are steps of
`doc/usd_compatibility_design.md`. `ERHE_geometry` keeps erhe and geogram
...
does build for Android: `doc/usd_compatibility_design.md` describes it. A USD
