...
`doc/usd_compatibility_design.md`; erhe extensions use erhe/glTF-context
...
item paths, `doc/usd_compatibility_design.md` step X2, in place of the
...
`doc/usd_compatibility_design.md`. `ERHE_geometry` keeps erhe/geogram
element naming; the primvar mapping is documented, not adopted.

The route that supersedes this trigger is `doc/usd_compatibility_design.md`:
...
- Per-instance override addressing (stable item paths inside a referenced
  ...
  critical path deliberately (`doc/usd_compatibility_design.md` steps M1
  and X2).
