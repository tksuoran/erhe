...
- `Animation` / `Animation_sampler` / `Animation_channel` -- Keyframe animation system supporting step, linear, and cubic spline interpolation of any animatable property of any item (see "Animation playback"). `Animation` is a typed prim (`erhe::Typed`, `doc/erhe_item.md` "Prim classes") with the fixed `typeName` token `Animation`, as `Skin` is with `Skin`.
...
no transform (`Typed`, `Scope`, see `doc/erhe_item.md`) and this library
owns the rest:
...
- `Xformable::handle_item_host_update()` registers the node with the scene host
  ...
  that drives it (see `doc/erhe_item.md`), so a `Scope` attached under a
...
(`doc/erhe_usd.md`, "Time samples"). A local-transform write-back
