...
- Every prim the conversion gives a transform to keeps the xformOp stack it
  ...
  (`doc/usd_compatibility_design.md` M8, `doc/erhe_scene.md` "Authored
...
`doc/erhe_scene_renderer.md`). A mesh's `primvars:displayColor` arrives
...
`doc/erhe_primitive.md`). Nothing of that input is then written as a
...
- The TARGET of an arc is any prim: an `Xformable`, a `Scope`, the `Typed`
  ...
  wrapper reproduces (`doc/editor_parsers.md`).
...
previous one (`doc/erhe_scene.md`, "Animation playback").
...
- lives in `src/editor/parsers/usd.{hpp,cpp}`; see `doc/editor_parsers.md`.
