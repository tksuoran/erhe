...
own subjects this one only refers to: `doc/draw_list_material_set.md` owns the
material buffer and texture heap a draw list binds, and
`doc/draw_list_performance_improvements.md` owns the cached per-primitive
...
- R8a (GPU buffer ownership and bind contract): `Draw_list_scene` owns its own
  **material** GPU state - a `Material_set`, and through it the material buffer
  and the texture heap (`doc/draw_list_material_set.md`) - and no other GPU
  ...
  which is the defect `doc/draw_list_material_set.md` exists to make
  unrepresentable.
- R9: Frame-varying per-entry GPU data (primitive transforms, joint matrices)
  ...
  (`doc/draw_list_performance_improvements.md`) is the upload source such a
  change would use.
...
- R12 (mesh / material edits): registered meshes are not immutable in practice.
  ...
  `doc/draw_list_material_set.md` follows that chain to the material set).
...
- R12b (live per-primitive upload inputs): per-primitive values edited in place
  ...
  (`doc/draw_list_performance_improvements.md`) supersedes the live reads for
...
- `Draw_list` (R13 / R14): its key, a contiguous vector of entries, the
  ...
  (`doc/draw_list_performance_improvements.md`), and the resolved-stages cache
...
- Flush site: `Editor::tick`, after transform propagation (so registration
  ...
  material-set update (`doc/draw_list_material_set.md` D6) runs right after it.
...
- Anything reachable from inside the rendergraph enqueues rather than acting:
  ...
  consumed them (`doc/draw_list_material_set.md` D1d).
