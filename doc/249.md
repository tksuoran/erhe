...
|---|---|---|
| Camera | none (multiple allowed is harmless; consider gating to one) | `erhe::scene::Camera(name)` |
| Light | none | `erhe::scene::Light(name)`, set `layer_id` from `scene_root->layers().light()->id` |
| Mesh (empty) | none | `erhe::scene::Mesh(name)`, no primitives; user adds geometry later |
...
- Multiple `Camera`/`Light`/`Mesh` attachments on one node are structurally
  allowed today; decide whether the add UI should gate them to one each
  (recommend: allow multiples, matching current data model; gates only where
  a duplicate is actually broken - `Node_physics`, `Layout`, `Grid`,
  `Frame_controller`, `Layout_item`).
- Removing a `Mesh` that a `Geometry_graph_mesh` or `Brush_placement`
  controls: the controlling attachment may recreate or dangle. Cheap answer:
  when a controlling attachment is present, disable removal of its controlled
  Mesh/Node_physics (tooltip explains why); removing the controller itself is
  fine.
