...
- `Material` -- PBR material: base color, roughness, metallic, emissive, texture samplers. It is a typed prim (`erhe::Typed`, `doc/erhe_item.md` "Prim classes") whose fixed `typeName` token is `Material`, USD's `UsdShadeMaterial`.
...
- `Material` is an `Item` with PBR properties and optional texture samplers. The five slot textures are object properties (`base_color_texture_property` ..., `register_member` over the `Material_data` slot's `texture_reference`, `doc/property_system.md` section 4.1): write a live material's slot through `set_base_color_texture()` and the other setters, `set_slot_texture()` for a slot held by reference, `set_slot_sampler()` for a slot's `Material_sampler_state`, `set_slot_uv_transform()` for a slot's rotation / offset / scale, `set_slot_texgen_mode()` for a slot's texgen source, or `set_data()` for a whole `Material_data` snapshot; every slot field (texture, transform, texgen source, sampler state) mirrors a property. `Material_sampler_state` is plain data (wrap, filters, mipmap mode, anisotropy, LOD bias); the renderer resolves the GPU `Sampler` from it. `erhe::primitive` links `erhe::graphics` publicly for the sampler enums.
- `Material_data` is closed: `get_data()` returns it const and the functions above are the only write path, so every change to what a material's GPU record and shader variant are built from passes through `on_property_changed`. A slot identifies itself by address, so a caller that holds one passes `get_data().texture_samplers.<slot>` to the setter.
- `Material::get_change_serial()` is a counter that advances on every such change: any property of the material (a local, style, reference, inherited or default change), and a texture graph bake that lands a new texture behind the `Texture_reference` a slot holds. The bake path is `erhe::graphics::Texture_reference_user`: a material registers with a slot's reference while it holds it, and a reference whose resolved texture changes over its lifetime (`editor::Graph_texture`) calls `on_referenced_texture_changed()` on each registered user. A reader that recorded a serial and reads the same one has nothing to re-derive.
- Which channel of a slot's texture each scalar input reads is a material property of its own: `metallic_channel`, `roughness_channel`, `occlusion_channel` and `opacity_channel`, each an `erhe::primitive::Texture_channel` (`r`, `g`, `b`, `a`, whose values are the component index). The defaults are glTF's fixed packing - metallic B and roughness G of the metallic-roughness slot, occlusion R of the occlusion slot, opacity A of the base color slot, erhe having no opacity slot of its own - so a glTF material authors none of them; a `UsdPreviewSurface` names the channel in its connection, which is where a non-default value comes from. The four travel to the shader as one `uvec4 texture_channels` of the material record, in that order, and the shader indexes the sampled texel with them.
- Where the base color and the fragment alpha come from is a material property of its own: `base_color_source` and `opacity_source`, each an `erhe::primitive::Material_input_source`. `value` is the material's own - the factor, times the slot's texture where one is bound, times the mesh's vertex color where the mesh carries one - and is what every glTF material and every plain `UsdPreviewSurface` wants; it is the default and the multiply is unchanged by this property. `vertex_color` is the mesh's color attribute alone: the factor and the texture of that input are not read at all. A `UsdPreviewSurface` input fed by a `UsdPrimvarReader` of `displayColor` / `displayOpacity` is what names it (`doc/erhe_usd.md`); glTF has no form for it, so a glTF material never authors one and the exporter writes nothing new. The pair travels to the shader as the `uvec2 input_sources` of the material record.
...
- Mesh optimization (`optimize_meshes`, on by default in the editor config) builds an
  ...
  edit's commit re-optimizes in the background. See `doc/meshoptimizer_integration.md`.
