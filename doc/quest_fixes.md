...
Draw_list_renderer follow-ups (formerly B12 + the Phase 6 prereq note) are
tracked in `doc/draw_list_renderer.md`'s "Quest-branch findings the swap does
NOT fix" section and at Phase 6, not here.
...
B2a. **[DONE - rename + contract change] "Lazy" naming for shader handles dropped** -- `Lazy_shader_handle` -> `Shader_stages_handle` (erhe_graphics); `Cached_shader_handle` -> `Variant_handle` (erhe_scene_renderer); `Render_pipeline_create_info::lazy_shader_stages` -> `shader_stages_handle`. The handle no longer promises deferred compile; its contract documents resolution timing as implementation-defined. The original B2a framed the Vulkan/Metal eager-ctor as a contract violation -- with the contract rewritten, the violation dissolves. See B2a-perf for the resolution of the residual cold-start question.

B2a-perf. **[CLOSED - eager construction is by design]** Vulkan / Metal `Render_pipeline_impl` ctor resolving `shader_stages_handle->shader_stages()` and immediately calling `vkCreateGraphicsPipelines` / `newRenderPipelineState` is the intended design, not a deferred problem. Pipeline-creation cost must land at times the application controls -- editor init, prewarm (`prewarm_all` / `Device::warmup_render_pipeline`), asset load -- so per-frame rendering does not stall on a first-bind compile. Deferring to first bind would trade a known, scheduled init cost for unpredictable mid-frame hitches, which is exactly what the prewarm path is built to prevent. The dynamic-pipeline hash + `device_impl.get_cached_pipeline(hash)` path in `vulkan_render_command_encoder.cpp` exists for runtime-state combinations that cannot be enumerated up-front; it is not a template for the static `Render_pipeline` API. No code change.
...
D6. **[DONE - reframed: no multiview-MSAA conflict exists]** **1-MSAA on OpenXR is a workaround** -- `config/editor/graphics_presets_openxr.json:5`. Root-cause: the "multiview-MSAA workaround" framing was wrong. The preset's `msaa_sample_count` only flows into single-view offscreen `Viewport_scene_view::Render_target` (handled correctly by the existing color-resolve attachment path in `render_target.cpp` and `vulkan_render_pass.cpp`), and `scene_builder.cpp:200` skips the default desktop viewport when OpenXR is on, so on standalone Quest the value is unreachable. The headset render pass at `headset_view.cpp:722-743` does not have any MSAA path of its own -- it attaches the OpenXR swapchain textures directly, and the swapchain's sample count is whatever the runtime put in `recommendedSwapchainSampleCount` (Quest reports 1). Multiview never combines with `sample_count > 1` anywhere in the current code. **Fix that landed:** preset restored to `msaa_sample_count = 4` matching the Medium desktop default, since the value only governs offscreen single-view MSAA which works. `Xr_session::get_swapchain_sample_count()` added so `prewarm.cpp` sources its `Warmup_target.sample_count` from the runtime rather than hard-coding 1; comment rewritten to describe the actual rationale (mirror the runtime's swapchain sample count). The "multiview-MSAA workaround per D6" reference in `prewarm.cpp` is gone.
...
M2. **[DONE] No persistence path for bxdf_model / use_* fields** -- `src/editor/io/gltf_fastgltf.cpp:1268, 1912, 2006`. glTF export and codegen JSON serializer don't touch the three new fields. **Fix:** wire `KHR_materials_unlit` import to `bxdf_model = unlit`; add the three new fields to the codegen serializer. **Verify:** roundtrip glTF preserves bxdf_model. *Resolved via glTF round-trip rather than codegen: export sets `material.unlit` for the unlit case, and `bxdf_model` / `use_circular_brushed_metal` / `use_aniso_control` (plus the existing `roughness_y`) ride in glTF material `extras`. Import reads them back. Material remains hand-authored; promoting it to codegen-managed is out of scope here.*
...
E1. **[DONE - partial; SHA256 verification moved to doc/todo.md] Gradle task ships stale validation layer .so** -- `android-project/app/build.gradle:171-195`. `outputs.upToDateWhen { layerSo.exists() }` did not invalidate on `validationLayerVersion` bump. **Landed:** `inputs.property("validationLayerVersion", validationLayerVersion)` so version bumps invalidate the task; the `-Pvulkan_validation_skip` branch explicitly deletes any staged `libs/arm64-v8a/libVkLayer*.so` so flipping to skip mode actually drops the layer (AGP `jniLibs` packaging would otherwise re-ship it). **Deferred:** SHA256 verification of the downloaded zip -- see `doc/todo.md` "SHA256-verify the Vulkan validation layer download" for the failure modes (supply-chain compromise, MITM, cache poisoning) and fix sketch.
...
E4. **[DONE] Hardcoded prewarm view counts** -- `src/editor/renderers/prewarm.cpp:89-99`. `view_counts.push_back(0u); view_counts.push_back(2u);`. **Fix:** source `max_view_count` from `Headset_view` / `Xr_session`; prewarm `{0, max_view_count}`. **Verify:** Varjo quad-view config exercises four-view prewarm.
...
E13. **[DONE] Dead code to delete** -- shipped in two passes. Init UI subset first (`Init_status_display::set_clear_color` removed; unused `texture_layer` param dropped from `render_text_overlay`). Final pass cleared the remaining four subsystems in a single commit:
   - **Renderers**: `Programs::load_programs`, the `programs_load_task` taskflow node + its `.succeed()` edge on `app_rendering_task`, `Programs::get_multiview`, and `m_handles_by_name` all deleted (`programs.{cpp,hpp}`, `editor.cpp`). Stale comments referencing those symbols updated in `app_context.hpp`, `app_rendering.cpp`, and `render_pipeline.hpp`. `m_programs` is built synchronously before any task spawns, so the dependency edge dropped cleanly.
   - **Graphics buffers**: bare-`Buffer_pool(transfer_queue, stream_index)` ctor and `Buffer_pool::add_existing_block` deleted (`buffer_pool.{cpp,hpp}`). `m_graphics_device` collapsed from `Device*` (null-in-manual-mode) to `Device&`; `m_block_create_info` collapsed from `std::optional<...>` to a plain value; manual-mode header doc dropped.
   - **Debug renderer**: `Debug_renderer_program_interface::padding0_offset` deleted (`debug_renderer.{cpp,hpp}`). The `view_block->add_float("_padding0")` call stays -- the std140 tail pad on the view block must still be declared in the GLSL block -- but the captured offset goes since nothing reads it.
   - **Mesh memory**: `Mesh_memory_config::vertex_buffer_size` / `index_buffer_size` deleted from the codegen def (`mesh_memory_config.py`); the regenerated `mesh_memory_config.hpp` drops both fields, and the orphan JSON keys are scrubbed from `config/editor/mesh_memory.json`, `config/example/mesh_memory.json`, `config/rendering_test/mesh_memory.json`. `doc/mesh_memory.md`'s "deprecated keys" footnote dropped.
