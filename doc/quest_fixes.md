...
B2b. **[DEFERRED - perf, not correctness] `Variant_handle` re-hits the cache mutex on every resolve** -- `src/erhe/scene_renderer/erhe_scene_renderer/variant_handle.cpp:28-31, 33-52`. `shader_stages()` calls `m_cache.get_or_compile(key)` on every call -- the cache takes its mutex even for hits. `multiview_shader_stages()` is worse: constructs a fresh `Shader_variant_key` per call (copies name, copies defines vector, re-runs canonicalize, re-registers the name in `Shader_name_registry` under another mutex) before hitting the cache. **Fix:** memoize the resolved `Shader_stages*` for both single-view and multiview inside the handle, under the existing `Shader_variant_cache::m_mutex` (lazy single-write, no atomics per CLAUDE.md). Reset from `Shader_variant_cache::clear()` -- which already fires on Material_change_operation and on Shader_monitor hot reload. **Verify:** sampling profile shows the variant-cache mutex no longer in the per-bind hot path.
