...
- `erhe::Item_host`'s prim hook (`doc/erhe_item.md` "Prim classes"),
...
sibling-unique (`doc/erhe_item.md` "Sibling-unique names") by the tree.
...
- **dropping** the file onto any row of the target scene in the Scene Hierarchy window, which places the texture as the hovered prim's last child (`doc/editor_windows.md` "Scene Hierarchy drag and drop").
