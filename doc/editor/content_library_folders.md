...
`doc/editor/content_library.md`, the property mechanics are
`doc/erhe/property_system.md` (D8 inheritance, D12 Add / Remove Property) and the
...
- R1 Folder tree. A kind scope holds resources and folders to any depth. A
  ...
  library's index.
- R2 Editing. The Scene Hierarchy's "Create" menu (`doc/editor/windows.md`
...
- D1b Resource names. Sibling-unique naming (`doc/erhe/item.md`
...
- D3 Moves. A move is `Item_parent_change_operation`, the same undoable
  ...
  `doc/editor/windows.md` "Scene Hierarchy drag and drop". A move within
...
- D8 Kind properties (R7). A `Scope`'s secondary owner type
  (`doc/erhe/property_system.md` D30) is the root owner type, as a `Style`'s is, so
...
- D9 Folder style. A folder carries a style like any item
  (`doc/editor/style_library.md` R4): the `style` row takes any style, the style's
...
- D5 Wire format. `ERHE_scene` carries `library_folders`: an array of
  ...
  name (doc/erhe/usd_compatibility_design.md E4d).
