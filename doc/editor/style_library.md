...
record; the layer mechanics are `doc/erhe/property_system.md` D25 (the style
layer) and D30 (secondary owner types), the library reference is
`doc/erhe/property.md`, and the folder counterpart is
`doc/editor/content_library_folders.md`.
...
- D2 Style item. `editor::Style` (`content_library/style.{hpp,cpp}`) is an
  ...
  `Styles` scope (`doc/erhe/usd_compatibility_design.md` U4). A style is clonable
