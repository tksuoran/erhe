...
- **`Content_library`** -- The per-scene index of a scene's resources
  ...
  prim tree (`doc/erhe/usd_compatibility_design.md` C5, U4), under the `Scope`
...
- **Kind scopes** -- `get_scope(kind_type_bit)` answers the `erhe::Scope` a
  ...
  (`doc/editor/scene_serialization.md`). A scene loaded from a file brings its kind
  ...
  (`doc/erhe/usd_compatibility_design.md` E4d); `get_scope()` looks for one in the
...
- `erhe::Item_host`'s prim hook (`doc/erhe/item.md` "Prim classes"),
...
(`doc/editor/content_library_folders.md`). Folders are selectable, the Properties
...
(`doc/editor/content_library_folders.md` D8): a Materials folder offers
...
sibling-unique (`doc/erhe/item.md` "Sibling-unique names") by the tree.
...
`doc/editor/style_library.md`): a style's secondary owner type is the root
...
- **dropping** the file onto any row of the target scene in the Scene Hierarchy window, which places the texture as the hovered prim's last child (`doc/editor/windows.md` "Scene Hierarchy drag and drop").
