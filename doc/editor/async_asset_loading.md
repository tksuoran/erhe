...
| Asset browser directory walk (startup and the Scan button) | the walk of `res/editor/assets` and `res/editor/scenes` runs on a worker and publishes batches the main thread attaches (doc/editor/asset_browser_scan.md) |
...
buffer sized to the request (doc/erhe/ring_buffer_memory.md) - so in
...
doc/editor/operations.md and doc/erhe/mesh_memory_deferred_free.md.
...
(doc/erhe/geogram.md), and the multi-stream buffer-mesh allocation groups and the
