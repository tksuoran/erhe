...
- **`Frame_controller`** -- Camera controller with 6DOF input axes (translate XYZ, rotate XYZ). A plain object owned by `Fly_camera_tool`, not an item in the scene: it names the camera node by `weak_ptr` and follows that node's transform through an `erhe::scene::Transform_observer_token` (`doc/erhe/scene.md` "Transform observers"), so a write by anyone else - a gizmo, the Properties window, an undo, MCP - is read back into the controller's pose and the next gesture continues from it.

- **`Node_physics`** (`node_physics.hpp`) -- The registration holder of a node's rigid-body values, keyed on `Node_physics.motion_mode` (`doc/erhe/property_system.md` section 4.26). `read_node_physics(node)` returns the effective values as a `Node_physics_data` record and `write_node_physics_create_info(node, create_info)` writes them; the body itself belongs to `Node_physics_system`. A convex hull / triangle shape keeps no reference to the geometry it was built from, so the node remembers it in `Node_physics.collision_mesh` (a weak object reference, D28): no value names the body's own mesh, and a value names a `Mesh` prim below the body, which is where both exporters then state the collider (`parsers/physics_export.cpp` makes it a physics entry of its own on that prim). `physics_import.cpp` sets it when a mesh collider of another prim folds into the body. The value records where the shape came from - setting it does not rebuild the shape - and a mesh that leaves the scene leaves the built shape standing, with the export falling back to the body's own mesh and one warning.
...
- **`Node_physics_system`** (`node_physics_system.hpp`) -- The per-scene owner of the rigid bodies' runtime state (doc/erhe/scene.md "Node systems", doc/erhe/property_system.md section 4.26), owned by `Scene_root` and added to its `Scene`, which drives it from the three node-system change sites. It keeps one `Node_physics_entry` per node, keyed by a raw `Node*`: the collision shape built for the node, the rigid body made from it, the create-info mirror the values feed, the world registration and the physics material / collision filter observers. A node has a live body exactly while it carries the group, is registered in this scene and its derived active bit is set. The shape outlives the body - an undo takes the node out of the scene and a redo puts it back without rebuilding it - so `on_node_unregistered` destroys the body and keeps the shape; an entry whose node died is purged when the next node registers. `set_collision_shape()` hands a node its shape and re-wraps it in the offset-center-of-mass wrapper the node's `center_of_mass_offset` asks for; `begin_interaction()` / `end_interaction()` override the body to kinematic for a drag without touching the node's motion mode; `teleport_to_node()` snaps a body to the node's pose with zero velocities. `get_bodies()` hands the simulation the entries with a body, parents before children.
