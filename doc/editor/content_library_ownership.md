...
`doc/editor/content_library.md`, and where a resource sits in the scene's prim
tree is `doc/erhe/usd_compatibility_design.md` C5 / U4.
...
- R3 Scene files stay self-contained - a saved scene opens correctly on
  another machine with no side files (`doc/editor/scene_serialization.md`).
...
- **Libraries as external glTF asset files.** That breaks R3 unless every
  ...
  (`doc/editor/asset_manager.md`), and it layers on top of per-scene ownership
