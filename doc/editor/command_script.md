...
| `scene.set_property`          | Handled directly by `run_startup_script()`: sets one registered erhe::property property of the item named `args.item` in the default scene (prims, content-library materials) to `args.value`, given in the property's string form (`erhe_property/property_string.hpp`: `true`/`false`, numbers, space-separated vector components, quaternions as `x y z w`, enumeration labels); omitting `value` resets to the default. `args.expression` instead of `value` drives the property by a formula (`doc/erhe/property_system.md` D22, e.g. `"{Driver/translation.y} * 2"`). Needs a scene (`scene.create` first). | yes (Property_set_operation) |
