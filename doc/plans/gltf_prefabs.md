...
(`doc/usd_compatibility_design.md` X2). `src/editor/prefabs/instance_structure.hpp`
...
layer (`doc/property_system.md` D33). `link_instance_to_template`
...
structure rule above applies (`doc/usd_compatibility_design.md` X2).
...
1. **`Prefab_library`** (editor part, app-wide): cache of parsed templates
   ...
   subtree (doc/usd_compatibility_design.md X1). Handles recursive loading (a
...
as-is, and doc/reloadable_asset_loads.md section 5 records why it was rejected:
