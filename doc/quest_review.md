...
- The new renderer keeps the GPU buffer pipeline unchanged (`Camera_buffer`, `Light_buffer`, `Material_buffer`, `Joint_buffer`, `Primitive_buffer`, `Draw_indirect_buffer`, `Texture_heap`, `Program_interface`, `Standard_shader_variants`). Findings against those subsystems remain live.
- The bucketing machinery inside `forward_renderer.cpp` (`Buffer_set`, `peek_mesh_buffers`, `Bucket_key`, `Bucket`, `bucket_primitives_by_buffers_and_variant`, `compute_bucket_variant_key`, the per-bucket `Debug_label` strings) goes away. Findings tied specifically to that machinery are marked **[OBSOLETE - Forward_renderer replacement]** inline below.
- `shadow_renderer` is NOT migrated in v1 (per `doc/draw_list_renderer.md` non-goals), so findings against `shadow_renderer.cpp` remain live. Where a single finding references both files, only the `forward_renderer.cpp` portion is obsoleted.
...
- **[high]** `Mesh_memory::get_or_create_format_pools` is not thread-safe -- `scene_renderer/erhe_scene_renderer/mesh_memory.cpp:56-78`. Inserts into `m_extra_format_pools` without a mutex. Today only called at init; first concurrent caller hits UB. **[OBSOLETE]** API removed; `Mesh_memory` now declares every non-default `Vertex_format` via an `extra_formats` ctor span and exposes only the read-only `get_format_pools` lookup. `m_extra_format_pools` is populated from the ctor body and never mutated afterwards, so no mutex is required.
- **[high]** Bucket key omits `Vertex_input_state*`, contradicting the design doc (`forward_renderer.cpp:142-154`, `shadow_renderer.cpp:41-53`). The renderer uses a single `parameters.vertex_input_state` for the whole call (`shadow_renderer.cpp:247`), so even with multiple formats registered the pipeline is still built for one input. The "exercises the multi-format pool registry end-to-end" claim in `editor.cpp:1023-1026` is misleading. **[OBSOLETE - Forward_renderer replacement]** the `forward_renderer.cpp` portion goes away with `Draw_list_renderer` (whose partition key is a `uint64_t` flag mask and whose buffer-set handling is a sorted-entry MDI-span scheme, not a bucket key). The `shadow_renderer.cpp` portion still applies until shadow_renderer is migrated separately.
...
- **[med]** `Buffer_set` duplicated verbatim across `forward_renderer.cpp:142-198` and `shadow_renderer.cpp:41-130`. Two file-local copies that will drift. Hoist into a shared header. **[OBSOLETE - Forward_renderer replacement]** the forward copy disappears with `Draw_list_renderer` (which keeps a sorted-by-buffer-set entry list internally rather than re-bucketing per frame). Shadow copy remains; revisit whether to share with `Draw_list_renderer`'s buffer-set key after the migration.
...
- **[high]** Dead config + UI knob shipped -- `graphics/definitions/graphics_config.py:60`, `scene/viewport_scene_view.hpp:167`. `use_draw_list_renderer` and `Renderer_choice` parsed / settable but never consumed. `_version` bumped 2 -> 4 (skipping 3) for a renderer not on disk. **[RECLASSIFY - scaffolding, not dead]** per `doc/draw_list_renderer.md` Phase 0, these *are* the intended config / UI hooks for the upcoming `Draw_list_renderer`. The remaining issues are real: (a) they should not land in main until the consumer does (or at least a fail-loud "renderer not yet available" log on selection), and (b) bumping `_version` 2 -> 4 (skipping 3) with no migration is still a CLAUDE.md `_version` violation regardless of who eventually reads the field.
...
- **[high]** Bucket variant key OR-merges material caps across primitives with no debug-mode trip -- `forward_renderer.cpp:233`. `compute_bucket_variant_key` ORs `boolean_mask` across every primitive in a bucket. Two materials with disjoint texture sets produce a single permissive variant. Benign today (texture flags unused), but as soon as `#ifdef ERHE_USE_*_TEXTURE` branches exist in the shader, primitives whose material lacks the texture will sample garbage handles. **[OBSOLETE - Forward_renderer replacement]** `compute_bucket_variant_key` and the per-bucket OR-merge belong to `Forward_renderer` and go away with `Draw_list_renderer`. `Draw_list_renderer`'s `Draw_entry` carries its own material shared_ptr per entry; variant selection happens per MDI-span instead of per merged bucket. The latent hazard for the *variant cache* itself (`Standard_shader_variants`) still needs a per-entry/per-material check at the new draw site -- track separately when the migration lands.
...
- **[high]** Per-bucket `Debug_label{fmt::format(...)}` leaks into `String_pool` monotonically -- `forward_renderer.cpp:548-561`, `shadow_renderer.cpp:330-341`. `Debug_label(std::string&&)` interns into a global `unordered_set<std::string>` (see `erhe_utility/debug_label.hpp:11-66`) with no eviction. Each frame, every bucket produces a unique string like `"bucket 1/3 prims=2 streams=3 mask=0x..."`. Each construction takes `std::scoped_lock` on a process-wide mutex inside the inner render loop. Gate on `Scoped_debug_group_impl::s_enabled` and use a stack-format buffer feeding the literal-array `Debug_label` ctor. **[OBSOLETE in part - Forward_renderer replacement]** the `forward_renderer.cpp` per-bucket label call site goes away with `Draw_list_renderer`; `shadow_renderer.cpp` keeps the same hazard. The underlying root cause -- `Debug_label(std::string&&)` interning unconditionally -- is in `erhe_utility/debug_label.hpp` and applies to *every* future debug-label site (including `Draw_list_renderer`'s per-MDI-span labels). Fix the interning, do not just wait for the renderer swap.
...
| 14 | `scene_renderer/mesh_memory.cpp:56-78` | `get_or_create_format_pools` not thread-safe. **[OBSOLETE]** API removed; registry is now setup-only via ctor `extra_formats`. |
| 15 | `forward_renderer.cpp:142-154`, `shadow_renderer.cpp:41-53` | Bucket key omits `Vertex_input_state*` -- single-pipeline renders defeat the multi-format design. **[OBSOLETE in part]** forward side disappears with `Draw_list_renderer`; shadow side remains. |
...
| 18 | `graphics/definitions/graphics_config.py:60`, `viewport_scene_view.hpp:167` | `use_draw_list_renderer` + `Renderer_choice` config / UI wired to nothing. **[RECLASSIFY]** intentional scaffolding for the planned `Draw_list_renderer` (see `doc/draw_list_renderer.md` Phase 0); residual concerns are the no-consumer landing and the unmigrated `_version` 2 -> 4 bump. |
...
| 21 | `forward_renderer.cpp:233` | Bucket variant key ORs material caps across primitives -- a future `#ifdef ERHE_USE_*_TEXTURE` branch will sample garbage. **[OBSOLETE]** `compute_bucket_variant_key` belongs to `Forward_renderer`; gone with the `Draw_list_renderer` replacement. Re-evaluate per-MDI-span variant selection in the new renderer. |
| 22 | `forward_renderer.cpp:548-561`, `shadow_renderer.cpp:330-341` | Per-frame `fmt::format` debug labels intern into global `String_pool` -- unbounded growth, global-mutex per bucket. **[OBSOLETE in part]** forward call site disappears; shadow remains and the underlying `Debug_label` interning hazard is renderer-agnostic -- fix in `erhe_utility/debug_label.hpp`, not by waiting for the renderer swap. |
...
- **Bucket / variant key misses fields it depends on** -- bucket key omits `Vertex_input_state*` and (at this tree state) `bxdf_model`; variant key has five always-true `USE_*_TEXTURE` axes. The *bucket* half of this theme is obsoleted by the planned `Draw_list_renderer` (no per-frame bucket key); the *variant* half (USE_\*_TEXTURE axes, bxdf_model) lives in `Standard_shader_variants` and survives the renderer swap.
