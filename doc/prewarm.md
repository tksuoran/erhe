...
- `Device::warmup_render_pipeline(const Render_pipeline_create_info&)`
  ...
  `Render_pipeline` from the supplied create_info and immediately
  discards it; on Vulkan the resulting binary is retained in the
  driver-level VkPipelineCache (`m_pipeline_cache`) because
  `Render_pipeline_impl`'s constructor calls
  `vkCreateGraphicsPipelines` with that cache. Subsequent
  constructions with the same shader-modules + pipeline-state tuple
  skip the IR-optimization pass and complete much faster on
  tile-based mobile drivers.
