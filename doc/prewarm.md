...
> **Status (2026-05-19): names changed; flow intact.** The prewarm path
> still moves first-frame shader compiles and Vulkan pipeline creation into
> init time; only the C++ surface around it changed:
>
> - `Standard_shader_variants` is gone. The variant cache is now
>   `Shader_variant_cache` in `shader_variant_cache.{hpp,cpp}`, keyed on
>   `Shader_key` (the renamed `Standard_variant_key`).
>   `Forward_renderer::prewarm_standard_variants(...)` is unchanged in
>   shape; the cache lookup it drives now resolves a `Shader_key`.
> - `compute_standard_variant_light_counts()` was replaced by
>   `compute_light_layer_partition()` in `shader_key.cpp`. The duplicated
>   "must stay bit-identical" tally in `Light_projections::apply` is gone;
>   both sites consume the same `Light_layer_partition` struct.
> - `Lazy_render_pipeline::get_pipeline_for(desc)` now takes additional
>   `shader_stages`, `vertex_input`, `vertex_format` arguments; pipeline
>   construction takes a `Base_render_pipeline_create_info` instead of the
>   composed create_info. Shadow_renderer's `prewarm_pipelines(...)`
>   accordingly dropped its `vertex_input_state` / `reverse_depth`
>   arguments; it now takes just the render-pass span.
> - `Render_pipeline_create_info::uses_standard_variants` is gone; the
>   prewarm walk visits every pipeline supplied via
>   `Prewarm_parameters::render_pipeline_states`. The
>   `extra_materials` + `fallback_vertex_format` knobs still apply.
>
> The why / where / OS-budget reasoning below is unchanged.
...
- `Forward_renderer::prewarm_standard_variants(const Prewarm_parameters&)`
  ...
  each `(pipeline, mesh_span)` pair and, for every requested view count
  (written into `key.multiview_view_count`), calls
  `Standard_shader_variants::get_or_compile(key)`. Mirrors the runtime
  gate exactly:
...
- Editor wiring (`src/editor/renderers/prewarm.cpp`). Sources the
  ...
  `Xr_session::enumerate_swapchain_formats()`, sample_count from
  `Xr_session::get_swapchain_sample_count()` (the runtime's
  `recommendedSwapchainSampleCount`; Quest reports 1), and usage
  flags mirroring `headset_view.cpp`'s per-frame render pass
  descriptor. Single-view (desktop mirror, main viewport offscreen)
  warmup is intentionally not wired because (a) those render passes
  are constructed lazily on first rendergraph execute and do not exist
  at this insertion point, and (b) the desktop path does not face the
  30 s OS interstitial budget that motivates the Quest warmup.
