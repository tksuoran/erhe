...
`doc/gl_worker_thread_contexts.md`, which describes the subsystem itself;
...
`doc/gl_worker_thread_contexts.md` states the rule in Traps as *"Subflow scope
...
signature to look for, and `doc/gl_worker_thread_contexts.md`'s warning about
...
throughput caveat in `doc/gl_worker_thread_contexts.md` ("a scope hoisted over
...
device access), and `doc/gl_worker_thread_contexts.md` still lists that code as
...
`doc/gl_worker_thread_contexts.md` warns in Traps: *"Do not trust derived
...
`doc/gl_worker_thread_contexts.md` lists "nested taskflows inside
