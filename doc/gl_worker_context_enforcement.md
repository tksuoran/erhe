...
Companion to `doc/gl_worker_thread_contexts.md`, which describes the GL
...
`doc/gl_worker_thread_contexts.md` states the placement rule in its Traps:
...
`doc/gl_worker_thread_contexts.md`'s warning about a deadlock that looks idle
...
`doc/gl_worker_thread_contexts.md` (a scope hoisted over expensive CPU work
...
`doc/gl_worker_thread_contexts.md` warns in its Traps: do not trust derived
...
`doc/gl_worker_thread_contexts.md` lists nested taskflows inside `parse_gltf`
