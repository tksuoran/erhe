...
) -> std::shared_ptr<Operation>
{
    ...
    if (add_default_light) {
        ...
        default_fill_light_node = fill_light;
...
auto save_scene_gltf(Scene_root& scene_root, const std::filesystem::path& path) -> bool
{
    ...
    const erhe::scene::Physics_description physics_data = build_physics_description(scene, scene_root.get_content_library().get(), &physics_items);
