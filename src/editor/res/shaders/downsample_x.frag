...
void main()
{
    ...
    vec3  prev_3   = weight_3 * texture(s_source, v_texcoord - vec2(offset_3, 0)).rgb;
    vec3  prev_2   = weight_2 * texture(s_source, v_texcoord - vec2(offset_2, 0)).rgb;
    vec3  prev_1   = weight_1 * texture(s_source, v_texcoord - vec2(offset_1, 0)).rgb;
    vec3  center   = weight_0 * texture(s_source, v_texcoord).rgb;
    vec3  next_1   = weight_1 * texture(s_source, v_texcoord + vec2(offset_1, 0)).rgb;
    vec3  next_2   = weight_2 * texture(s_source, v_texcoord + vec2(offset_2, 0)).rgb;
    vec3  next_3   = weight_3 * texture(s_source, v_texcoord + vec2(offset_3, 0)).rgb;
    ...
    //out_color      = vec4(texture(s_source, v_texcoord).rgb;, 1.0);
    out_color      = vec4(sum_d + sum_e, 1.0);
    //out_color      = vec4(v_texcoord, 0.0, 1.0);
    //out_color      = vec4(prev_2 + prev_1 + center + next_1 + next_2, 1.0);
}
