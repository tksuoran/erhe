...
// Input:  Linear Rec.709 scene-referred color (>= 0)
// Output: Linear scRGB (Rec.709 primaries, extended range)
// whiteLevel defines where highlights asymptotically compress to.
//   1.0  -> SDR white
//   >1.0 -> HDR white in scRGB

vec3 PBRNeutralToneMapping_scRGB(vec3 color, float whiteLevel) {
  const float startCompression = 0.8 - 0.04;
  const float desaturation = 0.15;
  ...
  const float d = whiteLevel - startCompression;

  float newPeak = whiteLevel - (d * d) / (peak + d - startCompression);
