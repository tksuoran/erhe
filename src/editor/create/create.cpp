    ...
    , m_window{imgui_renderer, imgui_windows, "Create", "create", [this]() { window_imgui(); }}
{
    ...
    // the brush leaves the editor (doc/editor/import_undo_reference_clearing.md).
...
auto Create::find_parent() -> std::shared_ptr<erhe::scene::Node>
{
    ...
    // doc/editor/active_item.md D6: the parent is the active node, else the
    // first selected node.
...
void Create::window_imgui()
{
...

    {
        ...
        if (ImGui::Button("Selected Mesh to Brush")) {
            ...
            content_library->add(m_context.asset_manager->create<Brush>(*scene_root, brush_create_info));
        }
    }
}
