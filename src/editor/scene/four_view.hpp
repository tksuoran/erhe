...
// The three axis-aligned orthogonal views of a four view.
...
// Attached to each orthogonal camera of a Four_view. Reports every transform
// write of the camera (fly camera, gizmo, Properties, undo) to the Four_view,
// which is what keeps the three views on one focus point without polling.
...
// Four linked views of one scene: a perspective view (not part of this class,
// it navigates freely) and three axis-aligned orthogonal views that share one
// focus point and one zoom. Each orthogonal camera sits at
// focus + axis * distance, looking at the focus. Moving one of them in its
// view plane moves the focus and with it the other two; changing the zoom of
// one applies to all three. See doc/editor/four_view.md.
class Four_view
{
public:
    static constexpr std::size_t axis_count = 3;

    Four_view(const std::shared_ptr<Scene_root>& scene_root, glm::vec3 focus, float view_height, float distance);
    ~Four_view() noexcept;
