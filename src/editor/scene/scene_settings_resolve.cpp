...
auto get_effective_viewport(const Editor_settings_config& editor_settings, const Scene_root& scene_root) -> const Viewport_config_data&
{
    const Scene_settings& overrides = scene_root.get_scene_settings();
    return overrides.viewport.has_value() ? overrides.viewport.value() : editor_settings.viewport;
}
