...
auto Mcp_server::query_draw_lists(const json& args) -> std::string
{
    ...
    // (doc/erhe/draw_list_material_set.md, phase 1). Bounded on purpose: a
    ...
    json result = {
        ...
        // Primitive record maintenance (doc/erhe/draw_list_performance_improvements.md):
        ...
        // Material persistence (doc/erhe/draw_list_material_set.md R10, R11):
...
auto Mcp_server::query_list_scenes(const json& args) -> std::string
{
    ...
    for (const auto& sr : m_context.app_scenes->get_scene_roots()) {
        ...
        scenes.push_back({
            ...
            // writes it in (doc/erhe/usd_compatibility_design.md E1).
...
auto Mcp_server::query_scene_nodes(const json& args) -> std::string
{
    ...
    // other prim (doc/erhe/usd_compatibility_design.md C5) and only the transformable
...
auto Mcp_server::query_scene_variants(const json& args) -> std::string
{
    ...
    for (const Variant_set& set : variant_table.get_sets()) {
        ...
        // (doc/erhe/usd_compatibility_design.md C9). `enclosing_selected` is
        ...
        variant_sets.push_back(
            json{
                ...
                {"enclosing_set_name",        set.enclosing_set_name},
                {"enclosing_variant_name",    set.enclosing_variant_name},
                {"enclosing_selected",        variant_table.is_live(set)},
...
// holder by (doc/editor/content_library_folders.md D5). A resource may sit under any
// prim (doc/erhe/usd_compatibility_design.md U4), which is why the second form is
// needed at all.
...
auto Mcp_server::find_items_by_ids(Scene_root& sr, const std::set<std::size_t>& target_ids) -> std::vector<std::shared_ptr<erhe::Item_base>>
{
    ...
    // (doc/erhe/usd_compatibility_design.md C5) and is registered nowhere.
