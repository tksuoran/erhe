...
{
    const std::vector<std::shared_ptr<Texture_graph_node>>& nodes = graph_texture.nodes();
...
{
    if (!root.is_object()) {
        log_graph_editor->warn("Graph texture load: root is not a JSON object");
        return false;
    }
    if (root.value("version", 0) != 1) {
        log_graph_editor->warn("Graph texture load: unsupported version {}", root.value("version", 0));
        return false;
    }
    ...
    for (const nlohmann::json& node_json : nodes_json) {
        ...
        const std::shared_ptr<Texture_graph_node> node = make_texture_graph_node(context, type_name);
        if (!node) {
            log_graph_editor->warn("Graph texture load: unknown node type '{}'", type_name);
            return false;
        }
    ...
    for (const nlohmann::json& link_json : links_json) {
        ...
        if ((source_node < 0) || (source_node >= node_count) || (sink_node < 0) || (sink_node >= node_count)) {
            log_graph_editor->warn("Graph texture load: link node index out of range");
            return false;
        }
        Texture_graph_node* source = new_nodes[static_cast<std::size_t>(source_node)].get();
        Texture_graph_node* sink   = new_nodes[static_cast<std::size_t>(sink_node)].get();
        if ((source_slot >= source->get_output_pins().size()) || (sink_slot >= sink->get_input_pins().size())) {
            log_graph_editor->warn("Graph texture load: link pin slot out of range");
            return false;
        }
        if (source->get_output_pins().at(source_slot).get_key() != sink->get_input_pins().at(sink_slot).get_key()) {
            log_graph_editor->warn("Graph texture load: link pin key mismatch");
            return false;
        }
    }

    // Clear any existing content, then register the new nodes and connect links.
    Texture_graph& graph = graph_texture.graph();
    std::vector<std::shared_ptr<Texture_graph_node>>& live_nodes = graph_texture.nodes();
    const std::vector<std::shared_ptr<Texture_graph_node>> existing = live_nodes; // copy
    for (const std::shared_ptr<Texture_graph_node>& node : existing) {
    ...
    for (const nlohmann::json& link_json : links_json) {
        ...
        if (link == nullptr) {
            log_graph_editor->warn("Graph texture load: link {} refused (cycle?)", link_index);
        }
        ++link_index;
    }
    graph.mark_dirty();
    log_graph_editor->info("Graph texture '{}' loaded ({} nodes, {} links)", graph_texture.get_name(), new_nodes.size(), links_json.size());
    return true;
}
