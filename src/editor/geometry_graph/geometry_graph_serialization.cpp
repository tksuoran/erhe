// Geometry graph save / load / clear (Geometry_graph_window member
...
//   {
//       "version": 1,
//       "nodes": [ { "type": "box", "position": [x, y], "parameters": { ... } }, ... ],
//       "links": [ { "source_node": 0, "source_slot": 0, "sink_node": 1, "sink_slot": 0 }, ... ]
//   }
// Links reference nodes by index into the "nodes" array.
...
auto Geometry_graph_window::save_graph(const std::filesystem::path& path) -> bool
{
    ...
    if (!stream) {
        log_graph_editor->warn("Geometry graph save: cannot open '{}' for writing", path.string());
        return false;
    }
    stream << root.dump(4);
...
auto Geometry_graph_window::load_graph(const std::filesystem::path& path) -> bool
{
    std::ifstream stream{path};
    if (!stream) {
        log_graph_editor->warn("Geometry graph load: cannot open '{}'", path.string());
        return false;
    }
    ...
    if (root.is_discarded() || !root.is_object()) {
        log_graph_editor->warn("Geometry graph load: '{}' is not valid JSON", path.string());
        return false;
    }
    const int version = root.value("version", 0);
    if (version != 1) {
        log_graph_editor->warn("Geometry graph load: '{}' has unsupported version {}", path.string(), version);
        return false;
    }
    ...
    for (const nlohmann::json& node_json : nodes_json) {
        ...
        if (!node) {
            log_graph_editor->warn("Geometry graph load: unknown node type '{}' in '{}'", type_name, path.string());
            return false;
        }
    ...
    for (const nlohmann::json& link_json : links_json) {
        ...
        if ((source_node < 0) || (source_node >= node_count) || (sink_node < 0) || (sink_node >= node_count)) {
            log_graph_editor->warn("Geometry graph load: link node index out of range in '{}'", path.string());
            return false;
        }
        Geometry_graph_node* source = content.nodes[static_cast<std::size_t>(source_node)].get();
        Geometry_graph_node* sink   = content.nodes[static_cast<std::size_t>(sink_node)].get();
