...
void Range_selection::end()
{
    ...
    // the active item of the range (doc/editor/active_item.md D3.2).
...
auto Selection::clear_selection(erhe::Item_host* host, const Active_item active_item) -> bool
{
    ...
    // (a plain click, a scoped re-selection) keeps it (doc/editor/active_item.md D2).
...
auto Selection::delete_items(const std::vector<std::shared_ptr<erhe::Item_base>>& items) -> bool
{
    ...
    for (const std::shared_ptr<erhe::Item_base>& item : items) {
        ...
        // (doc/erhe/usd_compatibility_design.md X2): an item inside an instance is
        ...
        // (doc/erhe/usd_compatibility_design.md U4), so a selected material is
...
{
    // The last listed item is the active item (doc/editor/active_item.md D3.2);
...
void Selection::set_active_item(const std::shared_ptr<erhe::Item_base>& item)
{
    ...
    // their final state (doc/editor/active_item.md D4).
...
void Selection::end_selection_change()
{
    ...
    // insert (doc/erhe/usd_compatibility_design.md U4) - would otherwise dispatch a
    ...
    // both in their final state (doc/editor/active_item.md D4).
    send_active_item_message(old_active_item, new_active_item);
}
...
void Selection::on_items_removed(const Removed_items& removed)
{
    ...
    // it is tested on its own (doc/editor/active_item.md D2). It is weak, but a
...
auto Selection::on_viewport_select_bone(const bool toggle) -> bool
{
    ...
    if (toggle) {
        if (was_selected) {
            // Same Ctrl-click rule as the object path (doc/editor/active_item.md D3.4).
...
void Selection::toggle_mesh_selection(const std::shared_ptr<erhe::scene::Mesh>& mesh, const bool was_selected, const bool clear_others)
{
    ...
    // pick of it selects that prim (doc/erhe/usd_compatibility.md, "Draw modes").
    ...
    if (clear_others) {
        ...
        if (effective_was_selected) {
            ...
            // deselects, and it stays active (doc/editor/active_item.md D3.4).
            if (m_active_item.lock() != item) {
                set_active_item(item);
                return;
            }
...
auto Selection::add_to_selection(const std::shared_ptr<erhe::Item_base>& item) -> bool
{
    ...
    // Every add makes the item active (doc/editor/active_item.md D3.1): the
...
{
    // doc/editor/active_item.md D7: the per-type map is kept for exactly the
