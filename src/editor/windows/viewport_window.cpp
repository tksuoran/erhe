...
{
    // Show the name of the Scene item this viewport is looking at; an
    // unbound viewport (no scene) is titled "Viewport". The
    ...
    static const std::string c_unbound_title{"Viewport"};
    const std::shared_ptr<Viewport_scene_view> viewport_scene_view = m_viewport_scene_view.lock();
    const std::shared_ptr<Scene_root> scene_root = viewport_scene_view ? viewport_scene_view->get_scene_root() : std::shared_ptr<Scene_root>{};
    const std::string& shown_name = (scene_root && !scene_root->get_name().empty())
        ? scene_root->get_name()
        : c_unbound_title;
    if (shown_name == m_shown_scene_name) {
        return;
    }
    ...
    m_shown_scene_name = shown_name;
    m_title = shown_name + m_title.substr(id_part);
}
