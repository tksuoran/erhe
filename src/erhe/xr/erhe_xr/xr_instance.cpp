...
) -> XrBool32
{
    ...
    // (V1) is not enabled. The specific message it raises mentions the
    // V1 enum literal XR_TYPE_GRAPHICS_BINDING_VULKAN_KHR; filter only
    // that one and let any other xrStructureTypeToString VUID escalate
    // through the callback so we don't silently swallow new bugs.
    ...
    const char* const message_text = (callback_data->message     != nullptr) ? callback_data->message     : "";
    const bool is_known_layer_false_positive =
        (std::strcmp(function_str, "xrStructureTypeToString") == 0) &&
        (std::strncmp(message_id, "VUID-xrStructureTypeToString-", 29) == 0) &&
        (std::strstr(message_text, "XR_TYPE_GRAPHICS_BINDING_VULKAN_KHR") != nullptr);
    if (is_known_layer_false_positive) {
        return XR_TRUE;
    }
...
#if !defined(XR_USE_PLATFORM_ANDROID)
        // Transient set_env failures (env table full, OS quirks) should
        // log a warning and keep the layer enabled rather than aborting
        // the editor. Mirrors set_env_or_warn in vulkan_device_init.cpp.
        if (!erhe::utility::set_env("XR_API_DUMP_EXPORT_TYPE", "text")) {
            log_xr->warn("Setting XR_API_DUMP_EXPORT_TYPE=text failed; api_dump output may go to stderr");
        }
        if (!erhe::utility::set_env("XR_API_DUMP_FILE_NAME", "logs/openxr.txt")) {
            log_xr->warn("Setting XR_API_DUMP_FILE_NAME=logs/openxr.txt failed; api_dump output may go to stderr");
        }
#endif
