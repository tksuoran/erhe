...
Mesh::Mesh()
    : Node_attachment{erhe::toolkit::Unique_id<Mesh>{}.get_id()}
{
}

Mesh::Mesh(const std::string_view name)
    : Node_attachment{name, erhe::toolkit::Unique_id<Mesh>{}.get_id()}
{
}
...
auto is_mesh(const Item* const item) -> bool
{
    ...
    return erhe::toolkit::test_all_rhs_bits_set(item->get_type(), erhe::Item_type::mesh);
}
