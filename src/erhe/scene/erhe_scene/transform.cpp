...
{
    m_matrix = erhe::math::create_orthographic(left, right, bottom, top, clip_range.z_near, clip_range.z_far);
...
{
    m_matrix = erhe::math::create_orthographic_centered(width, height, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}
...
{
    m_matrix = erhe::math::create_frustum(left, right, bottom, top, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}

void Transform::set_frustum_simple(const Clip_range clip_range, const float width, const float height)
{
    m_matrix = erhe::math::create_frustum_simple(width, height, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}

void Transform::set_perspective(const Clip_range clip_range, const float fov_x, const float fov_y)
{
    m_matrix = erhe::math::create_perspective(fov_x, fov_y, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}
...
{
    m_matrix = erhe::math::create_perspective_xr(fov_left, fov_right, fov_up, fov_down, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}

void Transform::set_perspective_vertical(const Clip_range clip_range, const float fov_y, const float aspect_ratio)
{
    m_matrix = erhe::math::create_perspective_vertical(fov_y, aspect_ratio, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}

void Transform::set_perspective_horizontal(const Clip_range clip_range, const float fov_x, const float aspect_ratio)
{
    m_matrix = erhe::math::create_perspective_horizontal(fov_x, aspect_ratio, clip_range.z_near, clip_range.z_far);
    m_inverse_matrix = glm::inverse(m_matrix);
}

Transform operator*(const Transform& lhs, const Transform& rhs)
{
    return Transform{lhs.get_matrix() * rhs.get_matrix()};
}
