...
def main() -> None:
    ...
    for arg in sys.argv[3:]:
        # Find separator colon.  On Windows, skip the drive-letter colon
        # (e.g. "D:") so that "D:/path:prefix" is not mis-split.
        start = 2 if (len(arg) >= 2 and arg[1] == ':') else 0
        sep_idx = arg.find(':', start)
        if sep_idx >= 0:
            extra_entries.append((Path(arg[:sep_idx]), arg[sep_idx + 1:]))
        else:
    ...
    for extra_dir, include_prefix in extra_entries:
        ...
        if include_prefix:
            ...
            for name in new_enums:
                get_enum_registry()[name].include_prefix = include_prefix
