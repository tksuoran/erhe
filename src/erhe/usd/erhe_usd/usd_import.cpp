...
// element table of doc/erhe/usd_compatibility.md read backwards: constant is one
...
// (doc/erhe/usd.md, "Time samples"): every one of them is an attribute
...
// (doc/erhe/usd.md, "Time samples"): a spline is a scalar value source
...
// token for token (doc/erhe/usd_compatibility.md, property system).
...
// attributes hold them (doc/erhe/usd_compatibility_design.md K1): `set_count` sets
...
public:
    ...
    void convert(const Stage::Impl& impl)
    {
        ...
        // (doc/erhe/usd.md, "Time samples"). A stage that samples
...
private:
    ...
    // doc/erhe/usd_compatibility_design.md M4). The conversions above ask the
    ...
    // (doc/erhe/usd.md, "Time samples"). One channel per sampled op of
    ...
    // (doc/erhe/usd.md, "Time samples"): a file's sampled `xformOp`s
    ...
    // (doc/erhe/usd_compatibility_design.md K1). A joint's animation is node
    ...
    // (doc/erhe/usd.md, "Time samples"). A spline is a scalar value
    ...
    // animation (doc/erhe/usd.md, "Time samples"). The samples are read
    ...
    )
    {
        ...
        // whatever the sampler says (doc/erhe/scene.md, "Animation
        // playback").
        erhe::scene::Animation_sampler sampler{erhe::scene::Animation_interpolation_mode::LINEAR};
        sampler.set(std::move(timestamps), std::move(values));
        animation->samplers.push_back(std::move(sampler));
    ...
    // erhe does not carry (doc/erhe/usd.md, "Time samples"): the
    ...
    // puts the editor's scene state there (doc/editor/scene_serialization.md, USD-
    ...
    // (doc/erhe/usd_compatibility.md, "Draw modes"). The schema is applied to a
    ...
    // arcs supplies (doc/erhe/usd_compatibility_design.md X1): USD composes the
    ...
    // The prim's authored xformOp stack (doc/erhe/usd_compatibility_design.md M8).
    ...
    // The authored opinions of one prim (doc/erhe/usd_compatibility_design.md I2).
    ...
    // `doubleSided` (doc/erhe/usd_compatibility.md, geometry attributes) lands on
    ...
    // the same vocabulary (doc/erhe/usd_compatibility_design.md M3). An unauthored
    ...
    // The `active` prim metadatum (doc/erhe/usd_compatibility_design.md X2). It is
    ...
    // The prim's composed specifier (doc/erhe/usd_compatibility_design.md X2). A
    ...
    // value addressed by its qualified name (doc/erhe/property_system.md D30). USD
    ...
    // (doc/erhe/usd_compatibility_design.md I1).
    ...
    // value is an authored value (doc/erhe/property_system.md D32).
    template <typename T>
    ...
    )
    {
        ...
        // (doc/erhe/usd.md, "Texture coordinates"). The USD identity
    ...
    // (doc/erhe/usd_compatibility.md, "Materials"). load_stage recorded every
    ...
    // (doc/erhe/usd_compatibility_design.md I2). An input the shader leaves at its
    ...
    void convert_materials()
    {
        ...
        for (std::size_t material_index = 0, end = m_scene->materials.size(); material_index < end; ++material_index) {
            ...
            // (doc/editor/content_library.md), which is what keeps the
            ...
            if (usd_material.openPBRShader.has_value()) {
                if (usd_material.surfaceShader.has_value()) {
                    ...
                    // UsdPreviewSurface input (doc/erhe/usd_node_graphs.md),
    ...
    ) const
    {
        ...
        for (std::size_t slot = 0; slot < 3; ++slot) {
            ...
            // top-left one (doc/erhe/usd.md, "Texture coordinates").
    ...
    // the element table of doc/erhe/usd_compatibility.md.
    ...
    void convert_meshes()
    {
        ...
        for (std::size_t mesh_index = 0, end = m_scene->meshes.size(); mesh_index < end; ++mesh_index) {
            ...
            // surface, which is `Gprim.display_color` (doc/erhe/usd_compatibility.md).
            ...
            // path names (doc/erhe/usd_compatibility_design.md X4).
    ...
    // (doc/erhe/usd_compatibility_design.md S1). Tydra converts no geometry for
    ...
    // file is (doc/erhe/usd_compatibility_design.md E4a). The mesh is not scene
    ...
    // other mesh of the file is (doc/erhe/usd_node_graphs.md section 4).
    ...
    void convert_cameras()
    {
        ...
        for (std::size_t camera_index = 0, end = m_scene->cameras.size(); camera_index < end; ++camera_index) {
            ...
            // (doc/erhe/usd_compatibility_design.md I2); the field of view and the
        ...
        for (std::size_t light_index = 0, end = m_scene->lights.size(); light_index < end; ++light_index) {
            ...
            switch (usd_light.type) {
                ...
                case Tydra_light::Type::Dome: {
                    ...
                    // scene's ambient light (doc/erhe/usd_compatibility.md,
            ...
            // (doc/erhe/usd_compatibility_design.md I2).
            ...
            // conversion is applied (see doc/erhe/usd_compatibility.md, Lights).
    ...
    // top-level imported nodes (doc/erhe/usd_compatibility.md, stage-level
    ...
    // ambient light (doc/erhe/usd_compatibility_design.md S1), which
    ...
    // its `typeName` names (doc/erhe/usd_compatibility_design.md C5). A prim the
    ...
    )
    {
        ...
        // `class` prim's is (doc/erhe/usd_compatibility_design.md S1, X3): the
    ...
    )
    {
        ...
        // (doc/erhe/usd_compatibility_design.md X3). Tydra reports it as a transform
        ...
        // (doc/erhe/usd_compatibility_design.md E4a): the caller makes the Brush item
        ...
        // (doc/erhe/usd_node_graphs.md R1): the caller rebuilds it from the
        ...
        // (doc/erhe/usd_compatibility_design.md K1).
        ...
        // (doc/erhe/usd_compatibility_design.md C5): its own xformOps are its
    ...
    // One `PointInstancer` prim as prims (doc/erhe/usd_compatibility_design.md S1).
    ...
    // (doc/erhe/usd.md, PointInstancer).
    ...
    // The prototypes one `class` prim holds (doc/erhe/usd_compatibility_design.md
    ...
    // (doc/erhe/usd_compatibility_design.md X3). The content a prim carries (a mesh,
    ...
    // that prim (doc/erhe/usd_compatibility_design.md U4): a material is a prim of
    ...
    // `Xform` prim per joint (doc/erhe/usd_compatibility_design.md K1). USD has no
    ...
    // point with (doc/erhe/usd_compatibility_design.md K1).
    ...
    // (doc/erhe/usd_compatibility_design.md K1). UsdSkel poses a bound point as
    ...
    // (doc/erhe/usd_compatibility_design.md C6). A variant contributes a composition arc while it is
    ...
    // of one of that prim's variant blocks (doc/erhe/usd_compatibility_design.md X4).
    ...
    // arcs bring in (doc/erhe/usd_compatibility_design.md C7). In LIVRPS such a
    ...
    // targets are what supply them (doc/erhe/usd_compatibility_design.md X1). True
    ...
    // contributed (doc/erhe/usd_compatibility_design.md X2): an `over` prim below
    ...
    )
    {
        ...
        for (const lightusd::PrimSpec& child : spec.children()) {
            if (relative_path.empty() && is_hoisted_variant_child(absolute_path, child.name())) {
                ...
                // hoist put here (doc/erhe/usd_compatibility_design.md C6). It
    ...
    // (doc/erhe/usd_compatibility_design.md M3): the same vocabulary, capitalized.
    ...
    // (doc/erhe/usd_compatibility_design.md X3), read before the prims are converted
    ...
    void collect_layer_composition(const std::string& path, const lightusd::PrimSpec& spec)
    {
        ...
        // (doc/erhe/usd_node_graphs.md R1): its `Shader` children are the
    ...
    // (doc/erhe/usd_compatibility_design.md X3) - and is converted as an ordinary
    ...
    // (doc/erhe/usd_compatibility_design.md E4a): its path and name, the density and
    ...
    // erhe texture graph (doc/erhe/usd_node_graphs.md 2.1).
    ...
    // What a node parameter travels as (doc/erhe/usd_node_graphs.md 2.2):
    ...
    // (doc/erhe/usd_node_graphs.md 2.3). An `inputs:` attribute carrying a
    ...
    // goes with it too (doc/erhe/usd_node_graphs.md 2.1).
    ...
    // asset from (doc/erhe/usd_node_graphs.md 2.3, section 4). The marker
    ...
    // (doc/erhe/usd_node_graphs.md R2). Tydra leaves such a slot unset -
    ...
    // ---------------------------------------------------------------------
    // Variant sets (doc/erhe/usd_compatibility_design.md X4)
    // ---------------------------------------------------------------------
    ...
    // below the prim carrying the set (doc/erhe/usd_compatibility_design.md X4).
    ...
    // (doc/erhe/usd_compatibility_design.md C6), so a path this tree cannot reach is
    ...
    // (doc/erhe/usd_compatibility_design.md C6). Every variant of the set is
    ...
    // became (doc/erhe/usd_compatibility_design.md X3). The caller resolves the
    ...
    // transform) pair it answers for (doc/erhe/usd_compatibility_design.md K1).
    ...
    // (doc/erhe/usd_compatibility_design.md E4a).
    ...
    // (doc/erhe/usd_compatibility_design.md X4).
    ...
    // (doc/erhe/usd_compatibility_design.md K1). Filled by index_skeletons and
