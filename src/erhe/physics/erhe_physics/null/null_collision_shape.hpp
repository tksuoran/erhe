...
class Null_tapered_capsule_shape : public Null_collision_shape
{
public:
    Null_tapered_capsule_shape(const Axis axis, const float bottom_radius, const float top_radius, const float length)
        : m_axis{axis}, m_bottom_radius{bottom_radius}, m_top_radius{top_radius}, m_length{length}
    {
    }

    ~Null_tapered_capsule_shape() noexcept override = default;
