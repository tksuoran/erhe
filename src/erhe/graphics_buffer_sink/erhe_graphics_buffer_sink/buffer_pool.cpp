...
auto Buffer_pool::grow_block_locked(const std::size_t min_capacity_bytes) -> Pool_block*
{
    ...
    // Reserve capacity up front so the two push_backs below cannot
    // throw bad_alloc mid-pair. If reserve throws, neither vector is
    // mutated; the unique_ptrs above drop the new Buffer and Pool_block
    // cleanly. Without the reserve, a bad_alloc on the second push_back
    // would leave the Buffer registered in m_owned_buffers but
    // unreferenced by any block, leaking the GPU allocation.
    m_owned_buffers.reserve(m_owned_buffers.size() + 1);
