...
// (Gltf_image_source retention; doc/editor/gltf_scene_roundtrip.md phase 0).
...
// (doc/editor/gltf_scene_roundtrip.md phase 3); the extras form is still
...
// of an ERHE_node payload (doc/erhe/usd_compatibility_design.md C5) - so the local
...
// that is not writable_when_sealed (doc/erhe/property_system.md). The flags
...
// content (doc/erhe/usd_compatibility_design.md X2). An entry without a "path" is
...
// (doc/editor/scene_serialization.md, prim classes): `prim_class` is "Scope" or
...
public:
    ...
    void parse_and_build()
    {
        ...
        // residency step creates (doc/editor/async_asset_loading_design.md step 3).
...
private:
    ...
    // (doc/editor/gltf_scene_roundtrip.md phase 2): POSITION accessor +
    ...
    // parse created (doc/erhe/usd_compatibility_design.md X4). The bindings are per
    ...
    // a class that carries no transform (doc/erhe/usd_compatibility_design.md C5):
...
// (doc/editor/async_asset_loading_design.md step 3). Everything below must run on a
...
auto parse_gltf(const Gltf_parse_arguments& arguments) -> Gltf_data
{
    ...
    // doc/erhe/usd_compatibility_design.md M4). Everything above filled the parsed
    ...
    // ERHE_material on materials (doc/editor/gltf_scene_roundtrip.md
...
private:
    ...
    // Geometry-normative primitive export (doc/editor/gltf_scene_roundtrip.md
    ...
    // (doc/editor/gltf_scene_roundtrip.md phase 3): the erhe-specific
    ...
    void record_material_extensions(const erhe::primitive::Material& material, const std::size_t gltf_material_index)
    {
        ...
        if (emit_style) {
            // The style item's name (doc/editor/style_library.md D4); quotes and
    ...
    // Extra unreferenced meshes (doc/editor/gltf_scene_roundtrip.md phase 3):
    ...
    // KHR_materials_variants (doc/erhe/usd_compatibility_design.md X4): the asset's
    ...
    // (doc/editor/gltf_scene_roundtrip.md phase 3): the FULL
    ...
    // --- Images / samplers / textures (doc/editor/gltf_scene_roundtrip.md phase 0) ---
    ...
    // --- Animations (doc/editor/gltf_scene_roundtrip.md phase 0) ---
    ...
    // --- Skins (doc/editor/gltf_scene_roundtrip.md phase 0) ---
    ...
    )
    {
        ...
        // (doc/erhe/usd_compatibility_design.md C5): a node without the field is an
    ...
    void process_child_nodes(const erhe::Hierarchy& erhe_node, const erhe::scene::Trs_transform& pre_transform, Index_vector& out_node_indices)
    {
        for (const std::shared_ptr<erhe::Hierarchy>& child : erhe_node.get_children()) {
            ...
            {
                // A Mesh is a prim (doc/erhe/usd_compatibility_design.md C5), so the
    ...
    // through to its children (doc/erhe/usd_compatibility_design.md C5).
    ...
    auto process_node(const erhe::scene::Node& erhe_node, const erhe::scene::Trs_transform& pre_transform = {}) -> std::size_t
    {
        ...
        // Exclusion hook (doc/editor/gltf_scene_roundtrip.md phase 3): an
...
auto Gltf_exporter::export_gltf() -> std::string
{
    ...
    // (doc/editor/gltf_scene_roundtrip.md phase 3); the extras remain parsed
