...
- `active` is the odd one out: its property is NOT inherits-flagged, because USD's prim `active` metadatum is the prim's own opinion, while the pruning it causes covers the whole subtree whatever a descendant says of itself (`doc/usd-compatibility-plan.md` X2). So `Item_flags::active` is the item's own value AND its inheritance parent's bit, recomputed for the item and - only when the bit moved - for the subtree by `rederive_active_flag_bits()`. That runs from the property changed callback and from the structural moves that change the parent (`Hierarchy::set_parent`, `Node_attachment::set_node`, `set_inheritance_container`); never per frame. `is_active()` reads it, and every consumer that requires `visible` requires `active` too - rendering, ID picking, raytracing, shadow casting, lightmap baking, DDGI and the physics world (`Node_physics` enters and leaves the Jolt world on the flip).
...
- `active_property`, `is_active()`, `rederive_active_flag_bits()` - owner type 0, default true, NOT `inherits`; the subtree effect is the derived bit, see "Item_flags" above
...
| `test_item_active.cpp` | 7 | Derived `active` bit follows the local value, an ancestor's value over the whole subtree, an item's own false value under an active ancestor and a tree move; `set_flag_bits` drops the bit; a copy re-derives from its own value |
