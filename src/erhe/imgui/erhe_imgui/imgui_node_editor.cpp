...
void ed::EditorContext::LoadSettings()
{
    ...
    {
        m_NavigateAction.NavigateTo(m_Style, m_Settings.m_VisibleRect, NavigateAction::ZoomMode::Exact, 0.0f);
    }
}
...
{
    m_FlowAnimationController.Flow(m_Style, link, direction);
}
...
void ed::FlowAnimationController::Flow(const Style& editorStyle, Link* link, FlowDirection direction)
{
    ...
    if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(ImGuiKey_F) && Editor->AreShortcutsEnabled())
    {
        ...
        if (!Editor->GetSelectedObjects().empty())
        {
            if (m_Reason != NavigationReason::Selection || m_LastSelectionId != Editor->GetSelectionId() || (zoomMode != NavigateAction::ZoomMode::None))
            {
                ...
                NavigateTo(Editor->GetStyle(), Editor->GetSelectionBounds(), zoomMode, -1.0f, NavigationReason::Selection);
            }
            else
        ...
        else if(auto hotObject = findHotObjectToZoom())
        {
            if (m_Reason != NavigationReason::Object || m_LastObject != hotObject || (zoomMode != NavigateAction::ZoomMode::None))
            {
                ...
                NavigateTo(Editor->GetStyle(), bounds, zoomMode, -1.0f, NavigationReason::Object);
            }
            else
                navigateToContent = true;
        }
        ...
        if (navigateToContent)
            NavigateTo(Editor->GetStyle(), Editor->GetContentBounds(), NavigateAction::ZoomMode::WithMargin, -1.0f, NavigationReason::Content);
    }
...
void ed::NavigateAction::NavigateTo(const Style& editorStyle, const ImRect& bounds, ZoomMode zoomMode, float duration, NavigationReason reason)
{
    if (ImRect_IsEmpty(bounds))
        return;

    if (duration < 0.0f)
        duration = editorStyle.ScrollDuration;
