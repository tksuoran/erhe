...
inline void Property_map<Key_type, Value_type>::transform(const glm::mat4 transform)
{
    ...
    const glm::mat4 cofactor = erhe::math::compute_cofactor(transform);
    ...
    if constexpr(transform_properties<Value_type>::is_transformable) {
        switch (m_descriptor.transform_mode) {
            ...
            case Transform_mode::cofactor_mat_mul_vec3_zero: {
                for (std::size_t i = 0, end = values.size(); i < end; ++i) {
                    if (present[i]) {
                        values[i] = apply_transform(values[i], cofactor, 0.0f);
                    }
                }
                break;
            }

            case Transform_mode::normalize_cofactor_mat_mul_vec3_zero: {
                for (std::size_t i = 0, end = values.size(); i < end; ++i) {
                    if (present[i]) {
                        values[i] = glm::normalize(apply_transform(values[i], cofactor, 0.0f));
                    }
                }
                break;
            }
        ...
        switch (m_descriptor.transform_mode) {
            ...
            case Transform_mode::cofactor_mat_mul_vec3_zero: {
                if constexpr (std::is_same_v<Value_type, glm::vec3>) {
                    for (std::size_t i = 0, end = source->values.size(); i < end; ++i) {
                        const Value_type source_value = source->values[i];
                        const Value_type result       = present[i] ? apply_transform(source_value, cofactor, 0.0f) : Value_type{};
                        values.push_back(result);
                    }
                }
                break;
            ...
            case Transform_mode::normalize_cofactor_mat_mul_vec3_zero   : {
                if constexpr (std::is_same_v<Value_type, glm::vec3>) {
                    for (std::size_t i = 0, end = source->values.size(); i < end; ++i) {
                        const Value_type source_value = source->values[i];
                        const Value_type result       = present[i] ? glm::normalize(apply_transform(source_value, cofactor, 0.0f)) : Value_type{};
                        values.push_back(result);
                    }
                }
                break;
