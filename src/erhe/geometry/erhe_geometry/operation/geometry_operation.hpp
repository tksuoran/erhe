...
protected:
    ...
    // need not be interpolated - either because this post_processing() run
    ...
    // regeneration_flags: see post_processing(). Channels regenerated by the
    // named process flags (smooth vertex normals, generated facet texcoords)
    // are skipped - their interpolated values would be overwritten.
    void interpolate_mesh_attributes(uint64_t regeneration_flags = 0);
