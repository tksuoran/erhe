...
Bake_transform::Bake_transform(const Geometry& source, const glm::mat4& transform, Geometry& destination)
...
auto bake_transform(const Geometry& source, const glm::mat4& transform) -> Geometry
{
    return Geometry{
        fmt::format("bake_transform({})", source.name),
        [&source, &transform](auto& result) {
            Bake_transform operation{source, transform, result};
        }
    };
}
