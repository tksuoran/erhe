...
{
public:
    Meta(const Geometry& source, Geometry& destination, const std::set<GEO::index_t>* selected_facets);

    void build();
};

Meta::Meta(const Geometry& source, Geometry& destination, const std::set<GEO::index_t>* selected_facets)
...
void Meta::build()
{
    ...
    // a selected src facet, add two triangles
    // (centroid, previous edge midpoint, corner) and (centroid, corner, next edge midpoint).
    //
    // Meta never moves vertices or edge midpoints off the original geometry
    ...
    // split; the unselected facets bordering the selection are re-emitted as spliced
    // n-gons so the seam stays watertight. With no active selection every edge /
    // facet qualifies, reproducing the whole-mesh behavior.
    ...
    // Split each selected facet into two triangles per corner. All edges of a
    // selected facet carry a midpoint, so the NO_VERTEX guard never fires here; it
    // is kept for safety.
    for (const GEO::index_t src_facet : source_mesh.facets) {
        if (!is_facet_selected(src_facet)) {
            continue;
...
#if !defined(NDEBUG)
    // Meta never smooths a vertex: every destination vertex carried over from a
