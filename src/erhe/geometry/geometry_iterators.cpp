...
void Point::for_each_corner(Geometry& geometry, std::function<void(Point_corner_context&)> callback)
{
         ...
         ++point_corner_id)
    {
        ...
        Point_corner_context context{geometry,
                                     point_corner_id,
                                     corner_id,
...
void Point::for_each_corner_const(const Geometry& geometry, std::function<void(Point_corner_context_const&)> callback) const
{
    for (Point_corner_id point_corner_id = first_point_corner_id,
         end = first_point_corner_id + corner_count;
         point_corner_id < end;
         ++point_corner_id)
    {
        ...
        Point_corner_context_const context{geometry,
                                           point_corner_id,
                                           corner_id,
                                           geometry.corners[corner_id]};
        callback(context);
...
void Point::for_each_corner_neighborhood(Geometry& geometry,
                                         std::function<void(Point_corner_neighborhood_context&)> callback)
{
    for (uint32_t i = 0; i < corner_count; ++i)
    {
        Point_corner_id prev_point_corner_id = first_point_corner_id + (corner_count + i - 1) % corner_count;
        Point_corner_id point_corner_id      = first_point_corner_id + i;
        Point_corner_id next_point_corner_id = first_point_corner_id + (i + 1) % corner_count;
        Corner_id       prev_corner_id       = geometry.point_corners[prev_point_corner_id];
        Corner_id       corner_id            = geometry.point_corners[point_corner_id];
        Corner_id       next_corner_id       = geometry.point_corners[next_point_corner_id];
        Point_corner_neighborhood_context context{geometry,
...
void Polygon::for_each_corner(Geometry& geometry, std::function<void(Polygon_corner_context&)> callback)
{
         ...
         ++polygon_corner_id)
    {
        ...
        Polygon_corner_context context{geometry,
                                       polygon_corner_id,
                                       corner_id,
                                       geometry.corners[corner_id]};
        callback(context);
...
void Polygon::for_each_corner_const(const Geometry& geometry, std::function<void(Polygon_corner_context_const&)> callback) const
{
    for (Polygon_corner_id polygon_corner_id = first_polygon_corner_id,
         end = first_polygon_corner_id + corner_count;
         polygon_corner_id < end;
    ...
    {
        Corner_id corner_id = geometry.polygon_corners[polygon_corner_id];
        Polygon_corner_context_const context{geometry,
                                             polygon_corner_id,
                                             corner_id,
                                             geometry.corners[corner_id]};
        callback(context);
...
void Polygon::for_each_corner_neighborhood(Geometry& geometry,
                                           std::function<void(Polygon_corner_neighborhood_context&)> callback)
{
    for (uint32_t i = 0; i < corner_count; ++i)
    {
        ...
        Polygon_corner_neighborhood_context context{geometry,
...
void Edge::for_each_polygon(Geometry& geometry, std::function<void(Edge_polygon_context&)> callback)
{
    for (Edge_polygon_id edge_polygon_id = first_edge_polygon_id,
         ...
         ++edge_polygon_id)
    {
        Polygon_id polygon_id = geometry.edge_polygons[edge_polygon_id];
        Edge_polygon_context context{geometry,
                                     edge_polygon_id,
                                     polygon_id,
                                     geometry.polygons[polygon_id]};
        callback(context);
        if (context.break_)
        {
            return;
