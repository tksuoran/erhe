...
auto GPU_ring_buffer::allocate_range(std::size_t required_alignment, Ring_buffer_usage usage, std::size_t byte_count) -> Buffer_range
{
    ERHE_PROFILE_FUNCTION();
...
auto GPU_ring_buffer_client::allocate_range(Ring_buffer_usage usage, std::size_t byte_count) -> Buffer_range
{
...
auto Instance::allocate_ring_buffer_entry(gl::Buffer_target buffer_target, Ring_buffer_usage usage, std::size_t byte_count) -> Buffer_range
{
    ...
    for (const std::unique_ptr<GPU_ring_buffer>& ring_buffer : m_ring_buffers) {
        ...
        if (available_byte_count_without_wrap >= byte_count) {
            return ring_buffer->allocate_range(required_alignment, usage, byte_count);
        }
    }
    ...
    return m_ring_buffers.back()->allocate_range(required_alignment, usage, byte_count);
}
