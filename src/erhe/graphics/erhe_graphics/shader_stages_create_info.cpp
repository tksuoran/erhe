...
) const -> std::string
{
    ...
    if (graphics_instance.info.gl_version < 460) {
        if (gl::is_extension_supported(gl::Extension::Extension_GL_ARB_shader_draw_parameters)) {
            ...
            sb << "#extension GL_ARB_shader_draw_parameters : require\n";
            sb << "#define gl_DrawID gl_DrawIDARB\n";
        } else {
            sb << "#define gl_DrawID erhe_DrawID\n";
        }
    }
    sb << "\n";
    ...
    if (!graphics_instance.info.use_multi_draw_indirect) {
        sb << "// Multi draw indirect emulation\n";
