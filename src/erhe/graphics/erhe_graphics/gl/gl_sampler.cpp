    ...
    , m_max_anisotropy   {create_info.max_anisotropy   }
{
    ...
    apply();
    m_debug_label = fmt::format("(S:{}) {}", gl_name(), create_info.debug_label);
...
auto Sampler_impl::to_gl(const Compare_operation compare_operation) -> int
{
...
auto Sampler_impl::to_gl(const Sampler_address_mode address_mode) -> int
{
    switch (address_mode) {
