...
{
    Material_set set;
    EXPECT_EQ(set.get_slot_count(), 2u) << "the reserved default slots must be covered by the GPU write";
    EXPECT_EQ(set.get_live_count(), 0u) << "the reserved slots hold no material";
    ...
    EXPECT_TRUE(slots[Material_set::vertex_colored_default_material_slot_index].alive);
    EXPECT_EQ(slots[Material_set::vertex_colored_default_material_slot_index].material, nullptr);
}
...
TEST(Material_set, default_slot_is_never_assigned)
{
    ...
    EXPECT_NE(set.add_ref(after).index, Material_set::vertex_colored_default_material_slot_index);
}
...
TEST(Material_set, library_sync_assigns_slots_in_order)
{
    ...
    // Slots 0 and 1 are the reserved default-material slots, so library
    // materials start at slot 2.
    EXPECT_EQ(set.get_slot(materials[0].get()), 2u);
    EXPECT_EQ(set.get_slot(materials[1].get()), 3u);
    EXPECT_EQ(set.get_slot(materials[2].get()), 4u);
    EXPECT_EQ(set.get_slot_count(), 5u);
}
...
TEST(Material_set, unrelated_removal_does_not_shift)
{
    ...
    const Material_list materials{make_material("a"), make_material("b"), make_material("c")};
    sync_library(set, materials);
    ASSERT_EQ(set.get_slot(materials[2].get()), 4u);
...
TEST(Material_set, unrelated_addition_does_not_shift)
{
    ...
    EXPECT_EQ(set.get_slot(materials[0].get()), 2u);
    EXPECT_EQ(set.get_slot(materials[1].get()), 3u);
    EXPECT_EQ(set.get_slot(added.get()),        4u);
}
...
TEST(Material_set, two_sets_are_independent)
{
    ...
    Material_set a;
    Material_set b;
    sync_library(a, Material_list{other, shared});  // shared -> slot 3
    sync_library(b, Material_list{shared});         // shared -> slot 2

    EXPECT_EQ(a.get_slot(shared.get()), 3u);
    EXPECT_EQ(b.get_slot(shared.get()), 2u);

    sync_library(b, Material_list{shared, other});
    EXPECT_EQ(a.get_slot(shared.get()), 3u) << "one set's sync moved another set's slot";
    EXPECT_EQ(a.get_slot(other.get()),  2u);
}
...
TEST(Material_set, get_slot_never_assigns)
{
    ...
    EXPECT_EQ(const_set.get_slot_count(), 3u); // the reserved slots and the library material
...
TEST(Material_set, slot_count_covers_holes)
{
    ...
    EXPECT_EQ(set.get_slot_count(), 5u) << "the GPU write has to cover the hole at slot 3";
    EXPECT_EQ(set.get_live_count(), 2u);
    const std::span<const Material_slot> slots = set.get_materials();
    ASSERT_GE(slots.size(), 5u);
    EXPECT_TRUE (slots[0].alive); // the reserved default slots
    EXPECT_TRUE (slots[1].alive);
    EXPECT_TRUE (slots[2].alive);
    EXPECT_FALSE(slots[3].alive);
    EXPECT_TRUE (slots[4].alive);
}
...
TEST(Material_set, object_churn_does_not_grow_the_slot_table)
{
    ...
    EXPECT_LE(set.get_slot_count(), 4u) << "slots accumulated across preview renders";
