...
{
    const Light_layer_partition partition = compute_light_layer_partition(layer.lights);

    Standard_variant_light_counts result{};
    result.directional_shadowmapped_count = static_cast<uint16_t>(partition.per_type_shadow[0]);
    result.directional_light_count        = static_cast<uint16_t>(partition.per_type_shadow[0] + partition.per_type_nonshadow[0]);
    result.spot_shadowmapped_count        = static_cast<uint16_t>(partition.per_type_shadow[1]);
    result.spot_light_count               = static_cast<uint16_t>(partition.per_type_shadow[1] + partition.per_type_nonshadow[1]);
    result.point_shadowmapped_count       = static_cast<uint16_t>(partition.per_type_shadow[2]);
    result.point_light_count              = static_cast<uint16_t>(partition.per_type_shadow[2] + partition.per_type_nonshadow[2]);
    return result;
}
