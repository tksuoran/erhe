    ...
    }
{
    ...
    m_view_block->add_struct("cameras", m_view_camera_struct.get(), static_cast<std::size_t>(m_view_count));
...
)
{
    ...
    if (multiview_path) {
        ERHE_VERIFY(static_cast<int>(multiview_views.size()) == m_view_count);
    } else {
    ...
    per_view_cameras.reserve(static_cast<std::size_t>(m_view_count));
    if (multiview_path) {
        ...
        // not read uninitialised entries when view_count > 1 but the
        // current call is single-view.
        while (per_view_cameras.size() < static_cast<std::size_t>(m_view_count)) {
    ...
    // Track the previously-bound edge SSBO (buffer + byte offset) so
    // consecutive dispatches that share the same source don't re-issue
    // the bind. The bind is the only SSBO state that can repeat across
    // dispatches (view UBO is fresh per dispatch; triangle SSBO is the
    // output and acquired anew each pass).
    ...
    for (Dispatch_entry& dispatch : m_dispatches) {
        ...
        // Skip the rebind if the previous dispatch already bound the
...
)
{
    ...
    if (multiview) {
        ...
        // pass viewMask. (The Base_render_pipeline cache is keyed on
