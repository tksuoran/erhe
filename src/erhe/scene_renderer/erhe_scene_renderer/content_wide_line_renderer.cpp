...
auto Content_wide_line_renderer::is_enabled() const -> bool { return m_enabled; }
...
)
{
    ...
    const erhe::scene::Camera_projection_transforms projection_transforms =
        camera.projection_transforms(viewport, true, erhe::math::Depth_range::zero_to_one);
    ...
    const float     clip_depth_direction = -1.0f; // reverse depth
