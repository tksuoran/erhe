...
{
    const uint64_t key = erhe::dataformat::compute_vertex_format_key(format);
    if (key == m_default_format_key) {
        return default_format_pools;
    }

    // Lookup + insert is mutex-guarded so taskflow workers (or any
    // future runtime caller) cannot race on m_extra_format_pools.
    // Format_pools' own ctor performs GPU work; we hold the lock
    // across construction to keep the (key -> pool) registration
    // atomic. Double-construction is acceptably rare and would
    // serialise behind this mutex anyway.
    std::lock_guard<std::mutex> lock{m_extra_format_pools_mutex};
