...
) -> erhe::graphics::Ring_buffer_range
{
    ...
    std::size_t primitive_count = 0;
    for (const erhe::scene::Mesh_primitive_ref& ref : primitive_refs) {
        if (!ref.mesh) {
            continue;
        }
        if (ref.mesh->get_node() == nullptr) {
            continue;
        }
        if (!filter(ref.mesh->get_flag_bits())) {
            continue;
        }
        if (ref.primitive_index >= ref.mesh->get_primitives().size()) {
            continue;
        }
        ++primitive_count;
    }
    ...
    for (const erhe::scene::Mesh_primitive_ref& ref : primitive_refs) {
        const std::shared_ptr<erhe::scene::Mesh>& mesh = ref.mesh;
        if (!mesh) {
            continue;
        }
        ...
        if (ref.primitive_index >= mesh_primitives.size()) {
            continue;
        }
        const erhe::scene::Mesh_primitive& mesh_primitive = mesh_primitives[ref.primitive_index];
        ...
        if (use_id_ranges) {
            m_id_ranges.push_back(
                Id_range{
                    ...
                    .length                          = count,
                    .mesh                            = mesh.get(),
                    .index_of_gltf_primitive_in_mesh = ref.primitive_index
                }
            );
            m_id_offset += count;
        }
