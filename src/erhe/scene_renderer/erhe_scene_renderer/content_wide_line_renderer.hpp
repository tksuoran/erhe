...
{
public:
    // max_view_count: size N of the cameras[N] array inside the view
    //   UBO block (and the per-view stride of the triangle SSBO).
    //   1 = single-view (default). >= 2 = stereo / OpenXR multiview.
    //   Mirrors Camera_interface::max_view_count; pass through from the
    ...
    // inputs (size must equal max_view_count from construction) and
    // leave camera nullptr; the compute writes max_view_count sets of
...
private:
    struct Dispatch_entry
    {
        ...
        // size = max_view_count * padded_edge_count * 6 * stride. The
    ...
    // repeated max_view_count times at the start of the view block).
