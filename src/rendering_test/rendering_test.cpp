...
public:
    ...
    void tick()
    {
        ...
        if (m_content_wide_line_renderer && m_content_wide_line_renderer->is_enabled()) {
            ...
            m_content_wide_line_renderer->compute(compute_encoder, cell_00, *m_camera.get());
        }
        ...
        update_swapchain_render_pass(full_width, full_height);
        {
            ...
            // Top-left (0,0): 3D scene with wide edge lines rendered directly to swapchain
            {
                ...
                // Compute-based edge lines (Metal path)
...
private:
    ...
    void create_test_textures()
    {
        ...
        const uint8_t red_pixels  [tex_size * tex_size * 4] = {};
        const uint8_t green_pixels[tex_size * tex_size * 4] = {};
        const uint8_t blue_pixels [tex_size * tex_size * 4] = {};
        ...
        auto make_solid_texture = [&](const char* name, uint8_t r, uint8_t g, uint8_t b) {
            ...
            m_graphics_device.upload_to_texture(
                *texture.get(), 0, 0, 0, tex_size, tex_size,
