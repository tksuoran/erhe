...
Quest's Vulkan loader has no validation layer of its own. Without it, GPU-side errors (descriptor mismatches, image-layout violations, multiview misuse, sync hazards, command-buffer lifecycle bugs) silently hang the editor on the first bad frame. With validation on, the same errors hit the device-error callback registered in `editor.cpp` (the `device_message` lambda; search for `Message_severity::error` -> `ERHE_FATAL`), so the process aborts with the offending VUID dumped to logcat (and, on desktop under `ERHE_AI_DRIVER=1`, to `logs/device_error.txt`).
...
| `VUID-VkShaderModuleCreateInfo-pCode-08737` (`DebugGlobalVariable: expected operand Variable must be ...`) | glslang emits `DebugGlobalVariable` with `Variable` operand referencing `OpConstantComposite` for global const composite arrays + the synthesised `gl_WorkGroupSize` | Fixed in the `tksuoran/glslang` fork pinned by the root `CMakeLists.txt` `CPMAddPackage` (CPM PATCHES is banned repo-wide). See `doc/reference/glslang_bug_report_debugglobalvariable.md` and the `erhe-quest-shader-failure` skill |
...
| `VUID-vkFreeCommandBuffers-pCommandBuffers-00047` (cb in pending state when freed) | Editor frees a command buffer the GPU has not finished with -- a frame-fence/lifecycle bug surfaced now that validation is on | Open as of 2026-05-02; did not reproduce in the 2026-08-14 5-min Quest soak |
| `VUID-VkPresentId2KHR-None-10820` (desktop only) | `present_image()` chains `VkPresentId2KHR` (device has present-id via VK_KHR_present_id2 only) but the swapchain lacked `VK_SWAPCHAIN_CREATE_PRESENT_ID_2_BIT_KHR`; aborted at first desktop present with validation on | Fixed in `c0ada98b` (2026-08-14) |
| `VUID-VkComputePipelineCreateInfo-layout-07988` (ray tracing) | `ray_trace.comp` samples `erhe_texture_heap` but both texture-heap set layouts were FRAGMENT-only; the failed pipeline was then dispatched null -> segfault (this was the long-standing "RT dies at startup on AMD" crash) | Fixed in `a53cbb53` (2026-08-14) |
| `VUID-vkGetAccelerationStructureDeviceAddressKHR-pInfo-09542` (ray tracing) | AS backing buffer created without `shader_device_address` usage | Fixed in `a4184de3` (2026-08-14) |
| `SYNC-HAZARD-READ-AFTER-WRITE` on mesh pools at `vkCmdBuildAccelerationStructuresKHR` (ray tracing) | `buffer_usage_to_vk_stage_access()` had no acceleration-structure cases, so post-upload barriers missed the AS-build stage; build inputs are read as `SHADER_READ` at `ACCELERATION_STRUCTURE_BUILD` (not `ACCELERATION_STRUCTURE_READ`, which is for source-AS reads) | Fixed in `ca65b736` (2026-08-14) |
| `VUID-vkCmdEndDebugUtilsLabelEXT-commandBuffer-recording` (end-label on cb not in recording state) | `Scoped_debug_group_impl` destructor recorded `vkCmdEndDebugUtilsLabelEXT` into the captured cb after the XR fan-out ended/submitted it mid-frame with the `Headset` / `Rendergraph::execute()` scopes still open. Aborted ~5 s after launch. | Fixed in `e6c6308f` (2026-08-14): regions spanning the cb split are queue-level labels (`Scoped_queue_debug_group`, selected via `Rendergraph_node::submits_command_buffer()`); `Command_buffer_impl::end()` now hard-verifies no cb-level scope is open. 5-min validation soak on Quest: 0 errors; remaining warnings are BestPractices-class (`ImageBarrierAccessLayout` on "ID Render color" TRANSFER_SRC transition with WRITE access mask is a real minor bug, unfixed). |
