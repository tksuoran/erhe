...
| `VUID-vkFreeCommandBuffers-pCommandBuffers-00047` (cb in pending state when freed) | Meta's OpenXR runtime calls `vkFreeCommandBuffers` on its previous-frame compositor cb (`vr-marker,frame_end,type,application,frame_num,N`) at the start of every `xrEndFrame` without waiting for it to retire. The fix is in `Xr_session::end_frame`, which now calls `Device::wait_for_frame(frame_index - 1)` (a `vkWaitSemaphores` on the device's frame-end timeline) just before `xrEndFrame`. Queue ordering then guarantees the runtime's prior compositor cb (queued before our `update_frame_completion` submit) has retired. | Fixed in commit `2e1aa892` |
